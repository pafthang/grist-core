...
describe("gristUrlState", function () {
  ...
  const dev = new UrlStateImpl({
    gristConfig: { org: undefined, pathOnly: true },
  });
  const single = new UrlStateImpl({
    gristConfig: { org: "mars", singleOrg: "mars", pathOnly: false },
  });
  const custom = new UrlStateImpl({
    gristConfig: { org: "mars", baseDomain: ".example.com" },
  });
  ...
  beforeEach(function () {
    mockWindow = {
      location: new URL("http://0.0.0.0:8080") as unknown as Location,
  ...
  it("should decode state in URLs correctly", function () {
    assert.deepEqual(prod.decodeUrl(new URL("http://0.0.0.0:8080")), {});
    assert.deepEqual(prod.decodeUrl(new URL("http://0.0.0.0:8080/ws/12")), {
      ws: 12,
    });
    assert.deepEqual(
      prod.decodeUrl(new URL("http://0.0.0.0:8080/o/foo/ws/12/")),
      { org: "foo", ws: 12 }
    );
    assert.deepEqual(
      prod.decodeUrl(new URL("http://0.0.0.0:8080/o/foo/doc/bar/p/5")),
      { org: "foo", doc: "bar", docPage: 5 }
    );

    assert.deepEqual(dev.decodeUrl(new URL("http://0.0.0.0:8080")), {});
    assert.deepEqual(dev.decodeUrl(new URL("http://0.0.0.0:8080/ws/12")), {
      ws: 12,
    });
    assert.deepEqual(
      dev.decodeUrl(new URL("http://0.0.0.0:8080/o/foo/ws/12/")),
      { org: "foo", ws: 12 }
    );
    assert.deepEqual(
      dev.decodeUrl(new URL("http://0.0.0.0:8080/o/foo/doc/bar/p/5")),
      { org: "foo", doc: "bar", docPage: 5 }
    );

    assert.deepEqual(single.decodeUrl(new URL("http://0.0.0.0:8080")), {
      org: "mars",
    });
    assert.deepEqual(single.decodeUrl(new URL("http://0.0.0.0:8080/ws/12")), {
      org: "mars",
      ws: 12,
    });
    assert.deepEqual(
      single.decodeUrl(new URL("http://0.0.0.0:8080/o/foo/ws/12/")),
      { org: "foo", ws: 12 }
    );
    assert.deepEqual(
      single.decodeUrl(new URL("http://0.0.0.0:8080/o/foo/doc/bar/p/5")),
      { org: "foo", doc: "bar", docPage: 5 }
    );

    assert.deepEqual(prod.decodeUrl(new URL("https://bar.example.com")), {
      org: "bar",
    });
    assert.deepEqual(
      prod.decodeUrl(new URL("https://bar.example.com/ws/12/")),
      { org: "bar", ws: 12 }
    );
    assert.deepEqual(
      prod.decodeUrl(new URL("https://foo.example.com/o/baz/ws/12")),
      { org: "baz", ws: 12 }
    );
    assert.deepEqual(prod.decodeUrl(new URL("https://foo.example.com/")), {
      org: "foo",
    });

    assert.deepEqual(dev.decodeUrl(new URL("https://bar.example.com")), {});
    assert.deepEqual(dev.decodeUrl(new URL("https://bar.example.com/ws/12/")), {
      ws: 12,
    });
    assert.deepEqual(
      dev.decodeUrl(new URL("https://foo.example.com/o/baz/ws/12")),
      { org: "baz", ws: 12 }
    );
    assert.deepEqual(dev.decodeUrl(new URL("https://foo.example.com/")), {});

    assert.deepEqual(single.decodeUrl(new URL("https://bar.example.com")), {
      org: "mars",
    });
    assert.deepEqual(
      single.decodeUrl(new URL("https://bar.example.com/ws/12/")),
      { org: "mars", ws: 12 }
    );
    assert.deepEqual(
      single.decodeUrl(new URL("https://foo.example.com/o/baz/ws/12")),
      { org: "baz", ws: 12 }
    );
    assert.deepEqual(single.decodeUrl(new URL("https://foo.example.com/")), {
      org: "mars",
    });

    // Trash page
    assert.deepEqual(
      prod.decodeUrl(new URL("https://bar.example.com/p/trash")),
      { org: "bar", homePage: "trash" }
    );

    // Billing routes
    assert.deepEqual(
      prod.decodeUrl(new URL("https://bar.example.com/o/baz/billing")),
      { org: "baz", billing: "billing" }
    );

    // API routes
    assert.deepEqual(
      prod.decodeUrl(new URL("https://bar.example.com/api/docs/bar")),
      { org: "bar", doc: "bar", api: true }
    );
    assert.deepEqual(
      prod.decodeUrl(new URL("http://0.0.0.0:8080/o/baz/api/docs/bar")),
  ...
  it("should decode query strings in URLs correctly", function () {
    assert.deepEqual(
      prod.decodeUrl(new URL("https://bar.example.com?billingPlan=a")),
      { org: "bar", params: { billingPlan: "a" } }
    );
    assert.deepEqual(
      prod.decodeUrl(
        new URL("https://foo.example.com/o/baz/ws/12?billingPlan=b")
      ),
      { org: "baz", ws: 12, params: { billingPlan: "b" } }
    );
    assert.deepEqual(
      prod.decodeUrl(
        new URL("https://bar.example.com/o/foo/doc/bar/p/5?billingPlan=e")
      ),
  ...
  it("should encode state in URLs correctly", function () {
    const localBase = new URL("http://0.0.0.0:8080");
    const hostBase = new URL("https://bar.example.com");

    assert.equal(prod.encodeUrl({}, hostBase), "https://bar.example.com/");
    assert.equal(
      prod.encodeUrl({ org: "foo" }, hostBase),
      "https://foo.example.com/"
    );
    assert.equal(
      prod.encodeUrl({ ws: 12 }, hostBase),
      "https://bar.example.com/ws/12/"
    );
    assert.equal(
      prod.encodeUrl({ org: "foo", ws: 12 }, hostBase),
      "https://foo.example.com/ws/12/"
    );

    assert.equal(
      dev.encodeUrl({ ws: 12 }, hostBase),
      "https://bar.example.com/ws/12/"
    );
    assert.equal(
      dev.encodeUrl({ org: "foo", ws: 12 }, hostBase),
      "https://bar.example.com/o/foo/ws/12/"
    );

    assert.equal(
      single.encodeUrl({ ws: 12 }, hostBase),
      "https://bar.example.com/ws/12/"
    );
    assert.equal(
      single.encodeUrl({ org: "foo", ws: 12 }, hostBase),
      "https://bar.example.com/o/foo/ws/12/"
    );

    assert.equal(
      prod.encodeUrl({ ws: 12 }, localBase),
      "http://0.0.0.0:8080/ws/12/"
    );
    assert.equal(
      prod.encodeUrl({ org: "foo", ws: 12 }, localBase),
      "http://0.0.0.0:8080/o/foo/ws/12/"
    );
    assert.equal(
      prod.encodeUrl({ org: "foo", doc: "bar" }, localBase),
      "http://0.0.0.0:8080/o/foo/doc/bar"
    );
    assert.equal(
      prod.encodeUrl({ org: "foo", doc: "bar", docPage: 2 }, localBase),
      "http://0.0.0.0:8080/o/foo/doc/bar/p/2"
    );

    assert.equal(
      dev.encodeUrl({ ws: 12 }, localBase),
      "http://0.0.0.0:8080/ws/12/"
    );
    assert.equal(
      dev.encodeUrl({ org: "foo", ws: 12 }, localBase),
      "http://0.0.0.0:8080/o/foo/ws/12/"
    );
    assert.equal(
      dev.encodeUrl({ org: "foo", doc: "bar" }, localBase),
      "http://0.0.0.0:8080/o/foo/doc/bar"
    );

    assert.equal(
      single.encodeUrl({ ws: 12 }, localBase),
      "http://0.0.0.0:8080/ws/12/"
    );
    assert.equal(
      single.encodeUrl({ org: "foo", ws: 12 }, localBase),
      "http://0.0.0.0:8080/o/foo/ws/12/"
    );
    assert.equal(
      single.encodeUrl({ org: "foo", doc: "bar" }, localBase),
      "http://0.0.0.0:8080/o/foo/doc/bar"
    );

    // homePage values, including the "Trash" page
    assert.equal(
      prod.encodeUrl({ homePage: "trash" }, localBase),
      "http://0.0.0.0:8080/p/trash"
    );
    assert.equal(
      prod.encodeUrl({ homePage: "all" }, localBase),
      "http://0.0.0.0:8080/"
    );
    assert.equal(
      prod.encodeUrl({ homePage: "workspace", ws: 12 }, localBase),
      "http://0.0.0.0:8080/ws/12/"
    );

    // Billing routes
    assert.equal(
      prod.encodeUrl({ org: "baz", billing: "billing" }, hostBase),
      "https://baz.example.com/billing"
    );

    // API routes
    assert.equal(
      prod.encodeUrl({ org: "baz", doc: "bar", api: true }, hostBase),
      "https://baz.example.com/api/docs/bar"
    );
    assert.equal(
      prod.encodeUrl({ org: "baz", doc: "bar", api: true }, localBase),
      "http://0.0.0.0:8080/o/baz/api/docs/bar"
    );
  });
  ...
  it("should encode state in billing URLs correctly", function () {
    ...
    assert.equal(
      prod.encodeUrl({ params: { billingPlan: "a" } }, hostBase),
      "https://bar.example.com/?billingPlan=a"
    );
    assert.equal(
      prod.encodeUrl({ ws: 12, params: { billingPlan: "b" } }, hostBase),
  ...
  describe("custom-domain", function () {
    it("should encode state in URLs correctly", function () {
      ...
      assert.equal(
        custom.encodeUrl({ org: "foo" }, hostBase),
        "https://foo.example.com/"
      );
      assert.equal(
        custom.encodeUrl({ ws: 12 }, hostBase),
        "https://www.martian.com/ws/12/"
      );
      assert.equal(
        custom.encodeUrl({ org: "foo", ws: 12 }, hostBase),
        "https://foo.example.com/ws/12/"
      );

      assert.equal(
        custom.encodeUrl({ ws: 12 }, localBase),
        "http://0.0.0.0:8080/ws/12/"
      );
      assert.equal(
        custom.encodeUrl({ org: "foo", ws: 12 }, localBase),
        "http://0.0.0.0:8080/o/foo/ws/12/"
      );
      assert.equal(
        custom.encodeUrl({ org: "foo", doc: "bar" }, localBase),
        "http://0.0.0.0:8080/o/foo/doc/bar"
      );
      assert.equal(
        custom.encodeUrl({ org: "foo", doc: "bar", docPage: 2 }, localBase),
        "http://0.0.0.0:8080/o/foo/doc/bar/p/2"
      );

      assert.equal(
        custom.encodeUrl({ org: "baz", billing: "billing" }, hostBase),
        "https://baz.example.com/billing"
      );
    });
    ...
    it("should encode state in billing URLs correctly", function () {
      ...
      assert.equal(
        custom.encodeUrl({ params: { billingPlan: "a" } }, hostBase),
        "https://www.martian.com/?billingPlan=a"
      );
      assert.equal(
        custom.encodeUrl({ ws: 12, params: { billingPlan: "b" } }, hostBase),
  ...
  it("should produce correct results with prod config", async function () {
    mockWindow.location = new URL(
      "https://bar.example.com/ws/10/"
    ...
    assert.equal(
      state.makeUrl({ org: "mars", doc: "DOC", docPage: 5 }),
      "https://mars.example.com/doc/DOC/p/5"
    );
  ...
  it("should produce correct results with single-org config", async function () {
    ...
    assert.equal(
      state.makeUrl({ org: "AB", doc: "DOC", docPage: 5 }),
      "https://example.com/o/AB/doc/DOC/p/5"
    );
  ...
  it("should produce correct results with custom config", async function () {
    ...
    assert.equal(state.makeUrl({ ws: undefined }), "https://example.com/");
    assert.equal(
      state.makeUrl({ org: "ab-cd", doc: "DOC", docPage: 5 }),
      "https://ab-cd.example.com/doc/DOC/p/5"
    );
  ...
  it("should support an update function to pushUrl and makeUrl", async function () {
    mockWindow.location = new URL(
      "https://bar.example.com/doc/DOC/p/5"
    ) as unknown as Location;
    const state = UrlState.create(
      null,
      mockWindow,
      prod
    ) as UrlState<IGristUrlState>;
    await state.pushUrl({
      params: { style: "singlePage", linkParameters: { foo: "A", bar: "B" } },
    });
    assert.equal(
      mockWindow.location.href,
      "https://bar.example.com/doc/DOC/p/5?style=singlePage&foo_=A&bar_=B"
    );
    state.loadState(); // changing linkParameters requires a page reload
    assert.equal(
      state.makeUrl((prevState) =>
        merge({}, prevState, { params: { style: "full" } })
      ),
    ...
    assert.equal(
      state.makeUrl((prevState) =>
        merge(omit(prevState, "params.style"), {
          docPage: 44,
          params: { linkParameters: { foo: "X" } },
        })
      ),
  ...
  describe("login-urls", function () {
    ...
    it("getLoginUrl should return appropriate login urls", function () {
      setWindowLocation("http://0.0.0.0:8080");
      assert.equal(getLoginUrl(), "http://0.0.0.0:8080/login?next=%2F");
    ...
    it("getLoginUrl should encode redirect url in next param", function () {
      setWindowLocation("http://0.0.0.0:8080/o/docs/foo");
      assert.equal(
        getLoginUrl(),
        "http://0.0.0.0:8080/o/docs/login?next=%2Ffoo"
      );
    ...
    it("getLoginUrl should skip encoding redirect url on signed-out page", function () {
      setWindowLocation("http://0.0.0.0:8080/o/docs/signed-out");
      assert.equal(getLoginUrl(), "http://0.0.0.0:8080/o/docs/login?next=%2F");
