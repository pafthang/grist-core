...
describe("gristUrls", function () {
  ...
  describe("encodeUrl", function () {
    it("should detect theme appearance override", function () {
      assertUrlDecode("http://0.0.0.0/?themeAppearance=light", {
        params: { themeAppearance: "light" },
      });

      assertUrlDecode("http://0.0.0.0/?themeAppearance=dark", {
        params: { themeAppearance: "dark" },
      });
    });

    it("should detect theme sync with os override", function () {
      assertUrlDecode("http://0.0.0.0/?themeSyncWithOs=true", {
        params: { themeSyncWithOs: true },
      });
    });

    it("should detect theme name override", function () {
      assertUrlDecode("http://0.0.0.0/?themeName=GristLight", {
        params: { themeName: "GristLight" },
      });

      assertUrlDecode("http://0.0.0.0/?themeName=GristDark", {
        params: { themeName: "GristDark" },
      });
    });

    it("should detect API URLs", function () {
      assertUrlDecode("http://0.0.0.0/o/docs/api/docs", { api: true });

      assertUrlDecode("http://public.getgrist.com/api/docs", { api: true });
    });
  });

  describe("parseFirstUrlPart", function () {
    it("should strip out matching tag", function () {
      assert.deepEqual(parseFirstUrlPart("o", "/o/foo/bar?x#y"), {
        value: "foo",
        path: "/bar?x#y",
      });
      assert.deepEqual(parseFirstUrlPart("o", "/o/foo?x#y"), {
        value: "foo",
        path: "/?x#y",
      });
      assert.deepEqual(parseFirstUrlPart("o", "/o/foo#y"), {
        value: "foo",
        path: "/#y",
      });
      assert.deepEqual(parseFirstUrlPart("o", "/o/foo"), {
        value: "foo",
        path: "/",
      });
    });
    ...
    it("should pass unchanged non-matching path or tag", function () {
      assert.deepEqual(parseFirstUrlPart("xxx", "/o/foo/bar?x#y"), {
        path: "/o/foo/bar?x#y",
      });
      assert.deepEqual(parseFirstUrlPart("o", "/O/foo/bar?x#y"), {
        path: "/O/foo/bar?x#y",
      });
      assert.deepEqual(parseFirstUrlPart("o", "/bar?x#y"), {
        path: "/bar?x#y",
      });
  ...
  describe("getHostType", function () {
    ...
    it('should interpret 0.0.0.0 as "native"', function () {
      assert.equal(getHostType("0.0.0.0", defaultOptions), "native");
      assert.equal(getHostType("0.0.0.0:8080", defaultOptions), "native");
    });
    ...
    it('should interpret base domain as "native"', function () {
      ...
      assert.equal(
        getHostType("foo.getgrist.com:8080", defaultOptions),
        "native"
      );
    });
    ...
    it('should interpret plugin domain as "plugin"', function () {
      assert.equal(
        getHostType("plugin.getgrist.com", defaultOptions),
        "plugin"
      );
      assert.equal(
        getHostType("PLUGIN.getgrist.com", {
          pluginUrl: "https://pLuGin.getgrist.com",
        }),
        "plugin"
      );
    ...
    it('should interpret doc internal url as "native"', function () {
      ...
      assert.equal(
        getHostType("doc-worker-123.internal", defaultOptions),
        "native"
      );
      assert.equal(
        getHostType("doc-worker-123.internal:8080", defaultOptions),
        "custom"
      );
      assert.equal(
        getHostType("doc-worker-124.internal", defaultOptions),
        "custom"
      );

      process.env.APP_DOC_INTERNAL_URL =
        "https://doc-worker-123.internal:8080/path";
      assert.equal(
        getHostType("doc-worker-123.internal:8080", defaultOptions),
        "native"
      );
      ...
      assert.equal(
        getHostType("doc-worker-124.internal:8080", defaultOptions),
        "custom"
      );
      assert.equal(
        getHostType("doc-worker-123.internal:8079", defaultOptions),
        "custom"
      );
    });
  });
  ...
  describe("getSlugIfNeeded", function () {
    it("should only return a slug when a valid urlId is used", function () {
      ...
      // no urlId
      assert.strictEqual(
        getSlugIfNeeded({ id: "1234567890abcdef", urlId: "", name: "Foo" }),
        undefined
      );
      assert.strictEqual(
        getSlugIfNeeded({ id: "1234567890abcdef", urlId: null, name: "Foo" }),
        undefined
      );
    });
    ...
    it("should leave only alphamerics after replacing reasonable unicode chars", function () {
      ...
      assert.strictEqual(
        getSlugIfNeeded({ id, urlId, name: "Hélène's résumé" }),
        "Helenes-resume"
      );
      assert.strictEqual(
        getSlugIfNeeded({ id, urlId, name: "Привіт, Їжак!" }),
        "Privit-Yizhak"
      );
      assert.strictEqual(
        getSlugIfNeeded({ id, urlId, name: "S&P500 is ~$4,894.16" }),
        "SandP500-is-dollar489416"
      );
    });
  });
});
