...
export class TestServerMerged extends EventEmitter implements IMochaServer {
  ...
  public async restart(reset: boolean = false, quiet = false) {
    ...
    if (reset) {
      if (process.env.TESTDIR) {
        ...
        this.testDir = path.join(
          tmpdir(),
          `grist_test_${username}_${this._name}_${workerIdText}`
        );
    ...
    // we restart.
    const testingSocket = path.join(
      this.testDir,
      `testing-${this._starts}.socket`
    );
    if (testingSocket.length >= 108) {
      // Unix socket paths typically can't be longer than this. Who knew. Make the error obvious.
      throw new Error(
        `Path of testingSocket too long: ${testingSocket.length} (${testingSocket})`
      );
    }
    ...
    const env: Record<string, string> = {
      ...
      GRIST_USER_ROOT: path.resolve(
        getAppRoot(),
        "test/fixtures/plugins/browserInstalledPlugins/"
      ),
      ...
      // The following line only matters for testing with non-0.0.0.0 URLs, which some tests do.
      ...
      ...(useSinglePort
        ...
        ? {
            ...
            DOC_WORKER_COUNT: "1",
            PORT: corePort,
            APP_UNTRUSTED_URL: `http://0.0.0.0:${untrustedPort}`,
        ...
        : {
            ...
            DOC_WORKER_COUNT: "5",
            PORT: "0",
            APP_UNTRUSTED_URL: "http://0.0.0.0:18096",
          }),
    ...
    this._serverUrl = `http://0.0.0.0:${port}`;
  ...
  public async isServerReady(): Promise<boolean> {
    try {
      return (await fetch(`${this._serverUrl}/status/hooks`, { timeout: 1000 }))
        .ok;
    } catch (err) {
      return false;
    }
  ...
  private _getDatabaseFile(): string {
    if (process.env.TYPEORM_TYPE === "postgres") {
      ...
      if (!db) {
        throw new Error("Missing TYPEORM_DATABASE");
      }
      return db;
    }
