...
describe("WebhookOverflow", function () {
  ...
  before(async function () {
    ...
    const webhookDetails: WebhookFields = {
      url: "https://0.0.0.0/WrongWebhook",
      eventTypes: ["update"],
      enabled: true,
      name: "test webhook",
      tableId: "Table2",
  ...
  async function getNumWaiting() {
    const cells = await gu.getVisibleDetailCells({
      col: "Status",
      rowNums: [1, 2],
    });
  ...
  async function overflowWebhook() {
    ...
    await gu.waitToPass(async () => {
      ...
      assert.include(
        toast,
        "New changes are temporarily suspended. Webhooks queue overflowed." +
...
export async function openDocumentSettings() {
  ...
  await driver
    .findContent(".grist-floating-menu a", "Document Settings")
    .click();
