...
export function addStatic(app: express.Express, rootDir?: string) {
  ...
  app.use(/^\/(grist-plugin-api.js)$/, (req, res) =>
    res.sendFile(req.params[0], { root: path.resolve(getAppRoot(), "static") })
  );
  app.use(
    express.static(rootDir || path.resolve(fixturesRoot, "sites"), {
...
export async function serveSomething(
