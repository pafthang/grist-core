...
describe("DocApi", function () {
  ...
  describe("should work with a merged server", async () => {
    setup("merged", async () => {
      const additionalEnvConfiguration = {
        ALLOWED_WEBHOOK_DOMAINS: `example.com,0.0.0.0:${webhooksTestPort}`,
        GRIST_DATA_DIR: dataDir,
      };
      home = docs = await TestServer.startServer(
        "home,docs",
        tmpDir,
        suitename,
  ...
  if (process.env.TEST_REDIS_URL) {
    describe("should work with a home server and a docworker", async () => {
      setup("separated", async () => {
        ...
        home = await TestServer.startServer(
          "home",
          tmpDir,
          suitename,
          additionalEnvConfiguration
        );
        docs = await TestServer.startServer(
          "docs",
          tmpDir,
          suitename,
          additionalEnvConfiguration,
  ...
  describe("QueryParameters", async () => {
    ...
    it("supports ascending sort", async function () {
      assert.deepEqual(
        applyQueryParameters(makeExample(), { sort: ["color"] }, null),
        {
          id: [8, 7, 9, 1, 3, 2],
    ...
    it("supports descending sort", async function () {
      assert.deepEqual(
        applyQueryParameters(makeExample(), { sort: ["-id"] }, null),
        {
          id: [9, 8, 7, 3, 2, 1],
    ...
    it("supports multi-key sort", async function () {
      assert.deepEqual(
        applyQueryParameters(makeExample(), { sort: ["-spin", "color"] }, null),
        {
          id: [8, 9, 1, 2, 7, 3],
    ...
    it("does not freak out sorting mixed data", async function () {
      ...
      assert.deepEqual(
        applyQueryParameters(example, { sort: ["mixed"] }, null),
        {
          mixed: [
            1,
            2.5,
...
function testDocApi() {
  async function generateDocAndUrl(docName: string = "Dummy") {
    const wid = (await userApi.getOrgWorkspaces("current")).find(
      (w) => w.name === "Private"
    )!.id;
  ...
  it("creator should be owner of a created ws", async () => {
    ...
    await userApi.updateOrgPermissions(ORG_NAME, {
    ...
    assert.equal(
      kiwiWsAccess.users.find((u) => u.email === kiwiEmail)?.access,
      "owners"
    );
    // Delete workspace.
  ...
  it("creator should be owner of a created doc", async () => {
    ...
    await userApi.updateWorkspacePermissions(ws1, {
      users: { [kiwiEmail]: "editors" },
    });
    ...
    assert.equal(
      kiwiDocAccess.users.find((u) => u.email === kiwiEmail)?.access,
      "owners"
    );
  ...
  it("should allow only owners to remove a document", async () => {
    ...
    await userApi.updateDocPermissions(doc1, {
    ...
    await userApi.updateDocPermissions(doc1, {
      users: { "kiwi@getgrist.com": "owners" },
    });
  ...
  it("should allow only owners to rename a document", async () => {
    ...
    await assert.isRejected(
      kiwiApi.renameDoc(doc1, "testrenameme2"),
      /Forbidden/
    );
  ...
  it("guesses types of new columns", async () => {
    ...
    const resp = await axios.post(
      `${serverUrl}/api/docs/${docIds.TestDoc}/apply`,
      userActions,
      chimpy
    );
    ...
    assert.deepEqual(
      (
        await axios.get(
          `${serverUrl}/api/docs/${docIds.TestDoc}/tables/GuessTypes/records`,
          chimpy
        )
      ).data,
      {
    ...
    assert.deepEqual(
      (
        await axios.get(
          `${serverUrl}/api/docs/${docIds.TestDoc}/tables/GuessTypes/columns`,
          chimpy
        )
  ...
  for (const content of ["with content", "without content"]) {
    for (const mode of ["logged in", "anonymous"]) {
      it(`POST /api/docs ${content} can create unsaved docs when ${mode}`, async function () {
        ...
        resp = await axios.get(
          `${serverUrl}/api/docs/${urlId}/tables/Table1/data`,
          user
        );
    ...
    it(`POST /api/docs ${content} can create saved docs in workspaces`, async function () {
      // Make a workspace.
      const chimpyWs = await userApi.newWorkspace(
        { name: "Chimpy's Workspace" },
        ORG_NAME
      );
      ...
      let resp = await axios.post(
        `${serverUrl}/api/docs`,
        ...(content === "with content" ? [formData, config] : [body, user])
      );
      assert.equal(resp.status, 200);
      const urlId = resp.data;
    ...
    it(`POST /api/docs ${content} fails if workspace is soft-deleted`, async function () {
      ...
      assert.equal(
        resp.data.error,
        "Cannot add document to a deleted workspace"
      );
  ...
  it("GET /docs/{did}/tables/{tid}/records handles errors and hidden columns", async function () {
    let resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.ApiDataRecordsTest}/tables/Table1/records`,
      chimpy
    );
    assert.equal(resp.status, 200);
    assert.deepEqual(resp.data, {
    ...
    resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.ApiDataRecordsTest}/tables/Table1/data`,
      chimpy
    );
    assert.equal(resp.status, 200);
    assert.deepEqual(resp.data, {
  ...
  it("GET /docs/{did}/tables/{tid}/columns retrieves columns", async function () {
    ...
    assert.deepEqual(respWithTableId.data, data);
    const respWithTableRef = await axios.get(
      `${serverUrl}/api/docs/${docIds.Timesheets}/tables/1/columns`,
      chimpy
    );
    assert.equal(respWithTableRef.status, 200);
    assert.deepEqual(respWithTableRef.data, data);
  ...
  it("GET/POST/PATCH /docs/{did}/tables and /columns", async function () {
    ...
    let resp = await axios.post(
      `${serverUrl}/api/docs/${docIds.Timesheets}/tables`,
      {
        tables: [
    ...
    resp = await axios.post(
      `${serverUrl}/api/docs/${docIds.Timesheets}/tables/NewTable2/columns`,
      {
        columns: [
          {},
          { id: "" },
    ...
    resp = await axios.post(
      `${serverUrl}/api/docs/${docIds.Timesheets}/tables/5/columns`,
      {
    ...
    resp = await axios.post(
      `${serverUrl}/api/docs/${docIds.Timesheets}/tables/NoSuchTable/columns`,
      { columns: [{}] },
      chimpy
    );
    assert.equal(resp.status, 404);
    ...
    resp = await axios.patch(
      `${serverUrl}/api/docs/${docIds.Timesheets}/tables`,
      {
        tables: [{ id: "Table3", fields: { tableId: "Table3_Renamed" } }],
      },
      chimpy
    );
    assert.equal(resp.status, 200);
    ...
    resp = await axios.patch(
      `${serverUrl}/api/docs/${docIds.Timesheets}/tables/Table2/columns`,
      {
        columns: [{ id: "A", fields: { colId: "A_Renamed" } }],
      },
      chimpy
    );
    assert.equal(resp.status, 200);
    ...
    // Repeat the same operation to check that it gives 404 if the table doesn't exist.
    resp = await axios.patch(
      `${serverUrl}/api/docs/${docIds.Timesheets}/tables/Table222/columns`,
      {
        columns: [{ id: "A", fields: { colId: "A_Renamed" } }],
      },
      chimpy
    ...
    resp = await axios.post(
      `${serverUrl}/api/docs/${docIds.Timesheets}/apply`,
      [
        [
          "AddRecord",
          "_grist_ACLResources",
          -1,
          { tableId: "NewTable2", colIds: "NewCol5" },
        ],
        [
          "AddRecord",
          "_grist_ACLResources",
          -2,
          { tableId: "NewTable2_2", colIds: "*" },
        ],
        [
          "AddRecord",
          "_grist_ACLRules",
    ...
    async function checkColumns(
      tableId: string,
      expected: { colId: string; label: string }[]
    ) {
      const colsResp = await axios.get(
        `${serverUrl}/api/docs/${docIds.Timesheets}/tables/${tableId}/columns`,
        chimpy
      );
    ...
    await checkColumns("Table2", [{ colId: "A_Renamed", label: "A" }]);
    await checkColumns("Table3_Renamed", [{ colId: "A", label: "A" }]);
    await checkColumns("NewTable1", [{ colId: "NewCol1", label: "NewCol1" }]);
    ...
    resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.Timesheets}/tables/NewTable2_2/columns`,
      chimpy
    );
    assert.equal(resp.status, 404);
    ...
    resp = await axios.post(
      `${serverUrl}/api/docs/${docIds.Timesheets}/tables/_grist_Tables/data/delete`,
      [2, 3, 4, 5, 6],
      chimpy
    );
    assert.equal(resp.status, 200);
    ...
    resp = await axios.post(
      `${serverUrl}/api/docs/${docIds.Timesheets}/tables/_grist_ACLRules/data/delete`,
      [2, 3],
      chimpy
    );
    assert.equal(resp.status, 200);
    resp = await axios.post(
      `${serverUrl}/api/docs/${docIds.Timesheets}/tables/_grist_ACLResources/data/delete`,
      [2, 3],
      chimpy
    );
    assert.equal(resp.status, 200);
  ...
  describe("/docs/{did}/tables/{tid}/columns", function () {
    ...
    describe("PUT /docs/{did}/tables/{tid}/columns", function () {
      ...
      async function checkPut(
        ...
        for (const [colId, expectedFields] of Object.entries(
          expectedFieldsByColId
        )) {
      ...
      it("should update existing columns and create new ones", async function () {
        await checkPut(
          [COLUMN_TO_ADD, COLUMN_TO_UPDATE],
          {},
          {
            NewA: { type: "Numeric", label: "A" },
            B: {},
            C: {},
            Foo: COLUMN_TO_ADD.fields,
          }
    ...
    describe("DELETE /docs/{did}/tables/{tid}/columns/{colId}", function () {
      it("should delete some column", async function () {
        ...
        assert.equal(
          resp.status,
          200,
          "Should succeed in requesting column deletion"
        );

        const listColResp = await axios.get(url, {
          ...chimpy,
          params: { hidden: true },
        });
        assert.equal(
          listColResp.status,
          200,
          "Should succeed in listing columns"
        );

        const columnIds = listColResp.data.columns
          .map(({ id }: { id: string }) => id)
          .sort();
      ...
      it("should return 404 if table not found", async function () {
        ...
        assert.equal(
          resp.data.error,
          'Table or column not found "NonExistingTable.A"'
        );
      });
      ...
      it("should return 404 if column not found", async function () {
        ...
        assert.equal(
          resp.data.error,
          'Table or column not found "Table1.NonExistingColId"'
        );
      });

      it("should forbid column deletion by viewers", async function () {
        const { url, docId } = await generateDocAndUrlForColumns(
          "ColumnDelete"
        );
        await userApi.updateDocPermissions(docId, {
          users: { "kiwi@getgrist.com": "viewers" },
  ...
  it("GET /docs/{did}/tables/{tid}/data returns 404 for non-existent doc", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/typotypotypo/tables/Table1/data`,
      chimpy
    );
    assert.equal(resp.status, 404);
  ...
  it("GET /docs/{did}/tables/{tid}/data returns 404 for non-existent table", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.Timesheets}/tables/Typo1/data`,
      chimpy
    );
    assert.equal(resp.status, 404);
  ...
  it("GET /docs/{did}/tables/{tid}/data supports filters", async function () {
    function makeQuery(filters: { [colId: string]: any[] }) {
      ...
      return axios.get(
        `${serverUrl}/api/docs/${docIds.Timesheets}/tables/Table1/data?${query}`,
        chimpy
      );
    }
  ...
  it("GET /docs/{did}/tables/{tid}/data returns matches /not found/ for bad table id", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/tables/Bad_Foo_/data`,
      chimpy
    );
    assert.equal(resp.status, 404);
  ...
  it("POST /docs/{did}/apply applies user actions", async function () {
    ...
    assert.deepEqual(
      (
        await axios.get(
          `${serverUrl}/api/docs/${docIds.TestDoc}/tables/Foo/data`,
          chimpy
        )
      ).data,
  ...
  it("POST /docs/{did}/apply respects document permissions", async function () {
    const userActions = [["AddTable", "FooBar", [{ id: "A" }]]];
    let resp: AxiosResponse;

    // as a guest chimpy cannot edit Bananas
    resp = await axios.post(
      `${serverUrl}/api/docs/${docIds.Bananas}/apply`,
      userActions,
      chimpy
    );
    assert.equal(resp.status, 403);
    ...
    resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.Bananas}/tables/FooBar/data`,
      chimpy
    );
    assert.equal(resp.status, 404);
    assert.match(resp.data.error, /not found/);
    ...
    // check that changes did not apply
    resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/tables/FooBar/data`,
      chimpy
    );
    assert.equal(resp.status, 404);
    assert.match(resp.data.error, /not found/);
  ...
  it("POST /docs/{did}/tables/{tid}/records adds records", async function () {
    let resp = await axios.post(
      `${serverUrl}/api/docs/${docIds.TestDoc}/tables/Foo/records`,
      {
        records: [
          { fields: { A: "John", B: 55 } },
          { fields: { A: "Jane", B: 0 } },
  ...
  it("parses strings in user actions", async () => {
    ...
    for (const queryParams of ["?noparse=1", ""]) {
      await axios.post(
        `${docUrl}/apply${queryParams}`,
        [["AddRecord", "Table1", null, { A: "$1" }]],
        chimpy
      );

      const response = await axios.post(
        `${recordsUrl}${queryParams}`,
        {
          records: [{ fields: { A: "$2" } }, { fields: { A: "$3" } }],
        },
        chimpy
      );
  ...
  describe("PUT /docs/{did}/tables/{tid}/records", async function () {
    ...
    it("should 404 for missing tables", async () => {
      checkError(
        404,
        /Table not found "Bad_Foo_"/,
        await axios.put(
          `${serverUrl}/api/docs/${docIds.TestDoc}/tables/Bad_Foo_/records`,
          { records: [{ require: { id: 1 } }] },
          chimpy
        )
      );
    });
    ...
    it("should 400 for missing columns", async () => {
      checkError(
        400,
        /Invalid column "no_such_column"/,
        await axios.put(
          `${serverUrl}/api/docs/${docIds.TestDoc}/tables/Foo/records`,
          { records: [{ require: { no_such_column: 1 } }] },
          chimpy
        )
      );
    });
    ...
    it("should validate request schema", async function () {
      ...
      const test = async (
        payload: any,
        error: { error: string; details: { userError: string } }
      ) => {
      ...
      await test(
        {},
        {
          ...
          details: { userError: "Error: body.records is missing" },
        }
      );
      await test(
        { records: 1 },
  ...
  describe("PATCH /docs/{did}/tables/{tid}/records", function () {
    ...
    it("allows CellValue as a field", async function () {
      ...
      const id = (await axios.post(url, { records: [{}] }, chimpy)).data
        .records[0].id;
      const testField = async (A?: CellValue, message?: string) => {
  ...
  describe("attachments", function () {
    it("POST /docs/{did}/attachments adds attachments", async function () {
      ...
      let resp = await axios.post(
        `${serverUrl}/api/docs/${docIds.TestDoc}/attachments`,
        formData,
    ...
    it("GET /docs/{did}/attachments/{id} returns attachment metadata", async function () {
      const resp = await axios.get(
        `${serverUrl}/api/docs/${docIds.TestDoc}/attachments/2`,
        chimpy
      );
      assert.equal(resp.status, 200);
    ...
    it("GET /docs/{did}/attachments/{id}/download downloads attachment contents", async function () {
      const resp = await axios.get(
        `${serverUrl}/api/docs/${docIds.TestDoc}/attachments/2/download`,
    ...
    it("GET /docs/{did}/attachments/{id}/download works after doc shutdown", async function () {
      ...
      let resp = await axios.post(
        `${serverUrl}/api/docs/${docIds.TestDoc}/force-reload`,
        null,
        chimpy
      );
      assert.equal(resp.status, 200);
    ...
    it("GET /docs/{did}/attachments/{id}... returns 404 when attachment not found", async function () {
      let resp = await axios.get(
        `${serverUrl}/api/docs/${docIds.TestDoc}/attachments/22`,
        chimpy
      );
      checkError(404, /Attachment not found: 22/, resp);
      resp = await axios.get(
        `${serverUrl}/api/docs/${docIds.TestDoc}/attachments/moo`,
        chimpy
      );
      checkError(
        400,
        /parameter cannot be understood as an integer: moo/,
        resp
      );
      resp = await axios.get(
        `${serverUrl}/api/docs/${docIds.TestDoc}/attachments/22/download`,
        chimpy
      );
      checkError(404, /Attachment not found: 22/, resp);
      resp = await axios.get(
        `${serverUrl}/api/docs/${docIds.TestDoc}/attachments/moo/download`,
        chimpy
      );
      checkError(
        400,
    ...
    it("POST/GET /docs/{did}/attachments respect document permissions", async function () {
      ...
      resp = await axios.get(
        `${serverUrl}/api/docs/${docIds.TestDoc}/attachments/3`,
        kiwi
      );
      checkError(403, /No view access/, resp);

      resp = await axios.get(
        `${serverUrl}/api/docs/${docIds.TestDoc}/attachments/3/download`,
        kiwi
      );
      checkError(403, /No view access/, resp);
    });
    ...
    it("POST /docs/{did}/attachments respects untrusted content-type only if valid", async function () {
      const formData = new FormData();
      formData.append("upload", "xyz", {
        filename: "foo",
        contentType: "application/pdf",
      });
      formData.append("upload", "abc", {
        filename: "hello.png",
        contentType: "invalid/content-type",
      });
      formData.append("upload", "def", {
        filename: "world.doc",
      ...
      resp = await axios.get(
        `${serverUrl}/api/docs/${docIds.TestDoc}/attachments/4/download`,
        chimpy
      );
      assert.equal(resp.status, 200);
      ...
      resp = await axios.get(
        `${serverUrl}/api/docs/${docIds.TestDoc}/attachments/5/download`,
        chimpy
      );
      assert.equal(resp.status, 200);
      ...
      resp = await axios.get(
        `${serverUrl}/api/docs/${docIds.TestDoc}/attachments/6/download`,
        chimpy
      );
      assert.equal(resp.status, 200);
    ...
    it("POST /docs/{did}/attachments/updateUsed updates timeDeleted on metadata", async function () {
      ...
      async function check(
        ...
        const actualUserData = resp.data.records.map(
          ({ id, fields: { Attached } }: ApiRecord) => ({ id, Attached })
        );
      ...
      const attachedValues = _.chunk(
        _.range(1, totalUsedAttachments + 1),
        attachmentsPerRecord
      ).map((arr) => ["L", ...arr]);
      await check(
        [
          ...
          [
            "BulkAddRecord",
            "Table1",
            arrayRepeat(numRecords, null),
            { Attached: attachedValues },
          ],
          [
            "BulkAddRecord",
            "_grist_Attachments",
            arrayRepeat(totalAttachments, null),
            {},
          ],
        ],
        attachedValues.map((Attached, index) => ({ id: index + 1, Attached })),
        _.range(totalAttachments).map((index) => ({
          id: index + 1,
          deleted: index >= totalUsedAttachments,
        }))
      );
    });
    ...
    it("POST /docs/{did}/attachments/removeUnused removes unused attachments", async function () {
      ...
      async function checkAttachmentIds(ids: number[]) {
        ...
        assert.equal(resp.status, 200);
        assert.deepEqual(
          resp.data.records.map((r: any) => r.id),
          ids
        );
      }
      ...
      resp = await axios.patch(
        `${docUrl}/tables/_grist_Attachments/records`,
        {
          records: [
            {
              id: 1,
              fields: { timeDeleted: Date.now() / 1000 - 8 * 24 * 60 * 60 },
            }, // 8 days ago, i.e. expired
            {
              id: 2,
              fields: { timeDeleted: Date.now() / 1000 - 6 * 24 * 60 * 60 },
            }, // 6 days ago, i.e. not expired
          ],
        },
        chimpy
      );
      ...
      resp = await axios.post(
        `${docUrl}/attachments/verifyFiles`,
        null,
        chimpy
      );
      assert.equal(resp.status, 200);
      ...
      resp = await axios.post(
        `${docUrl}/attachments/removeUnused?verifyfiles=1`,
        null,
        chimpy
      );
      assert.equal(resp.status, 200);
  ...
  it("GET /docs/{did}/download serves document", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/download`,
      chimpy
    );
    assert.equal(resp.status, 200);
  ...
  it("GET /docs/{did}/download/csv serves CSV-encoded document", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.Timesheets}/download/csv?tableId=Table1`,
      chimpy
    );
    assert.equal(resp.status, 200);
    assert.equal(
    ...
    const resp2 = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/download/csv?tableId=Foo`,
      chimpy
    );
  ...
  it("GET /docs/{did}/download/csv with header=colId shows columns id in the header instead of their name", async function () {
    ...
    const csvResp = await axios.get(
      `${docUrl}/download/csv?tableId=Table1&header=colId`,
      chimpy
    );
  ...
  it("GET /docs/{did}/download/csv respects permissions", async function () {
    // kiwi has no access to TestDoc
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/download/csv?tableId=Table1`,
      kiwi
    );
    assert.equal(resp.status, 403);
    assert.notEqual(
  ...
  it("GET /docs/{did}/download/csv returns 404 if tableId is invalid", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/download/csv?tableId=MissingTableId`,
      chimpy
    );
    assert.equal(resp.status, 404);
  ...
  it("GET /docs/{did}/download/csv returns 404 if viewSectionId is invalid", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/download/csv?tableId=Table1&viewSection=9999`,
      chimpy
    );
    assert.equal(resp.status, 404);
    assert.deepEqual(resp.data, {
      error: "No record 9999 in table _grist_Views_section",
    });
  });

  it("GET /docs/{did}/download/csv returns 400 if tableId is missing", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/download/csv`,
      chimpy
    );
    assert.equal(resp.status, 400);
    assert.deepEqual(resp.data, {
      error: "tableId parameter should be a string: undefined",
    });
  });

  it("GET /docs/{did}/download/table-schema serves table-schema-encoded document", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/download/table-schema?tableId=Foo`,
      chimpy
    );
    assert.equal(resp.status, 200);
    const expected = {
  ...
  it("GET /docs/{did}/download/table-schema serves table-schema-encoded document with header=colId", async function () {
    const { docUrl, tableUrl } = await generateDocAndUrl(
      "tableSchemaWithColIdAsHeader"
    );
    const columns = [
      {
        id: "Some_ID",
    ...
    const setupColResp = await axios.put(
      `${tableUrl}/columns`,
      { columns },
      { ...chimpy, params: { replaceall: true } }
    );
    assert.equal(setupColResp.status, 200);

    const resp = await axios.get(
      `${docUrl}/download/table-schema?tableId=Table1&header=colId`,
      chimpy
    );
    assert.equal(resp.status, 200);
    const expected = {
  ...
  it("GET /docs/{did}/download/table-schema respects permissions", async function () {
    // kiwi has no access to TestDoc
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/download/table-schema?tableId=Table1`,
      kiwi
    );
    assert.equal(resp.status, 403);
    assert.deepEqual(resp.data, { error: "No view access" });
  ...
  it("GET /docs/{did}/download/table-schema returns 400 if tableId is missing", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/download/table-schema`,
      chimpy
    );
    assert.equal(resp.status, 400);
    assert.deepEqual(resp.data, {
  ...
  it("GET /docs/{did}/download/xlsx serves XLSX-encoded document", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.Timesheets}/download/xlsx?tableId=Table1`,
      chimpy
    );
    assert.equal(resp.status, 200);
  ...
  it("GET /docs/{did}/download/xlsx respects permissions", async function () {
    // kiwi has no access to TestDoc
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/download/xlsx?tableId=Table1`,
      kiwi
    );
    assert.equal(resp.status, 403);
    assert.deepEqual(resp.data, { error: "No view access" });
  ...
  it("GET /docs/{did}/download/xlsx returns 404 if viewSectionId is invalid", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/download/xlsx?tableId=Table1&viewSection=9999`,
      chimpy
    );
    assert.equal(resp.status, 404);
    assert.deepEqual(resp.data, {
  ...
  it("GET /docs/{did}/download/xlsx returns 200 if tableId is missing", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.TestDoc}/download/xlsx`,
      chimpy
    );
    assert.equal(resp.status, 200);
    assert.notEqual(resp.data, null);
  ...
  it("POST /workspaces/{wid}/import handles empty filenames", async function () {
    ...
    const resp = await axios.post(
      `${worker1.url}/api/workspaces/${wid}/import`,
      { uploadId: uploadId1 },
  ...
  it("handles /s/ variants for shares", async function () {
    ...
    let resp: AxiosResponse;
    resp = await axios.post(
      `${serverUrl}/api/docs/${docId}/apply`,
      userActions,
      chimpy
    );
    assert.equal(resp.status, 200);
    ...
    resp = await axios.get(
      `${serverUrl}/api/s/${key}/tables/Table1/records`,
      chimpy
    );
    assert.equal(resp.status, 200);

    resp = await axios.get(
      `${serverUrl}/api/docs/${key}/tables/Table1/records`,
      chimpy
    );
    assert.equal(resp.status, 404);

    resp = await axios.get(
      `${serverUrl}/api/docs/${SHARE_KEY_PREFIX}${key}/tables/Table1/records`,
      chimpy
    );
    assert.equal(resp.status, 200);

    resp = await axios.get(
      `${serverUrl}/api/s/${key}xxx/tables/Table1/records`,
      chimpy
    );
    assert.equal(resp.status, 404);
  });
  ...
  it("document is protected during upload-and-import sequence", async function () {
    ...
    let wid = (await kiwiApi.getOrgWorkspaces("current")).find(
      (w) => w.name === "Big"
    )!.id;
    let resp = await axios.post(
      `${worker2.url}/api/workspaces/${wid}/import`,
      { uploadId: uploadId1 },
      configForUser("Kiwi")
    );
    assert.equal(resp.status, 403);
    ...
    wid = (await userApi.getOrgWorkspaces("current")).find(
      (w) => w.name === "Private"
    )!.id;
    resp = await axios.post(
      `${worker1.url}/api/workspaces/${wid}/import`,
  ...
  it("limits parallel requests", async function () {
    ...
    await axios.post(
      `${serverUrl}/api/docs/${docIds.Timesheets}/force-reload`,
      null,
      chimpy
    );
    const reqs = [...Array(30).keys()].map((i) =>
      axios.get(
        `${serverUrl}/api/docs/${docIds.Timesheets}/tables/Table1/data`,
        chimpy
      )
    ...
    assert.lengthOf(
      responses.filter((r) => r.status === 429),
      20
    );
  });
  ...
  it("allows forced reloads", async function () {
    ...
    if (hasHomeApi) {
      ...
      resp = await axios.post(
        `${serverUrl}/api/housekeeping/docs/${docIds.Timesheets}/force-reload`,
        null,
        support
      );
      assert.equal(resp.status, 200);
  ...
  it("allows assignments", async function () {
    let resp = await axios.post(
      `${serverUrl}/api/docs/${docIds.Timesheets}/assign`,
      null,
      chimpy
    );
    assert.equal(resp.status, 200);
    ...
    if (hasHomeApi) {
      ...
      resp = await axios.post(
        `${serverUrl}/api/housekeeping/docs/${docIds.Timesheets}/assign`,
        null,
        support
      );
      assert.equal(resp.status, 200);
  ...
  it("honors urlIds", async function () {
    ...
    const ws1 = (await userApi.getOrgWorkspaces("current"))[0].id;
    const doc1 = await userApi.newDoc(
      { name: "testdoc1", urlId: "urlid1" },
      ws1
    );
    try {
      // Make sure an edit made by docId is visible when accessed via docId or urlId
      await axios.post(
        `${serverUrl}/api/docs/${doc1}/tables/Table1/data`,
        {
          A: ["Apple"],
          B: [99],
        },
      ...
      resp = await axios.get(
        `${serverUrl}/api/docs/urlid1/tables/Table1/data`,
        chimpy
      );
      assert.equal(resp.data.A[0], "Apple");
  ...
  it("filters urlIds by org", async function () {
    ...
    const doc2 = await nasaApi.newDoc(
      { name: "testdoc2", urlId: "urlid" },
      ws2
    );
    try {
      // Place a value in "docs" doc
      await axios.post(
        `${serverUrl}/o/docs/api/docs/urlid/tables/Table1/data`,
        {
          A: ["Apple"],
          B: [99],
        },
      ...
      await axios.post(
        `${serverUrl}/o/nasa/api/docs/urlid/tables/Table1/data`,
        {
          A: ["Orange"],
          B: [99],
        },
      ...
      assert.equal(resp.data.A[0], "Apple");
      resp = await axios.get(
        `${serverUrl}/api/docs/${doc2}/tables/Table1/data`,
        chimpy
      );
  ...
  it("allows docId access to any document from merged org", async function () {
    ...
    try {
      ...
      let resp = await axios.post(
        `${serverUrl}/o/nasa/api/docs/${doc1}/tables/Table1/data`,
        {
          A: ["Apple"],
          B: [99],
        },
      ...
      resp = await axios.post(
        `${serverUrl}/o/docs/api/docs/${doc2}/tables/Table1/data`,
        {
          A: ["Orange"],
          B: [99],
        },
      ...
      resp = await axios.post(
        `${serverUrl}/o/pr/api/docs/${doc2}/tables/Table1/data`,
        {
          A: ["Orange"],
          B: [99],
        },
  ...
  it("GET /docs/{did}/replace replaces one document with another", async function () {
    ...
    await userApi.updateDocPermissions(doc2, {
      users: { "kiwi@getgrist.com": "editors" },
    });
    await userApi.updateDocPermissions(doc3, {
      users: { "kiwi@getgrist.com": "viewers" },
    });
    await userApi.updateDocPermissions(doc4, {
      users: { "kiwi@getgrist.com": "owners" },
    });
    try {
      // Put some material in doc3
      let resp = await axios.post(
        `${serverUrl}/o/docs/api/docs/${doc3}/tables/Table1/data`,
        {
          A: ["Orange"],
        },
        chimpy
      ...
      resp = await axios.post(
        `${serverUrl}/o/docs/api/docs/${doc2}/replace`,
        {
          sourceDocId: doc3,
        },
        kiwi
      ...
      resp = await axios.post(
        `${serverUrl}/o/docs/api/docs/${doc1}/replace`,
        {
          sourceDocId: doc3,
        },
        kiwi
      ...
      resp = await axios.post(
        `${serverUrl}/api/docs/${doc3}/apply`,
        [
          [
            "AddRecord",
            "_grist_ACLResources",
            -1,
            { tableId: "Table1", colIds: "A" },
          ],
          [
            "AddRecord",
            "_grist_ACLRules",
            ...
            {
              resource: -1,
              aclFormula: "user.Access not in [OWNER]",
              permissionsText: "-R",
            },
          ],
        ],
      ...
      assert.equal(resp.status, 200);
      resp = await axios.post(
        `${serverUrl}/o/docs/api/docs/${doc4}/replace`,
        {
          sourceDocId: doc3,
        },
        kiwi
      ...
      resp = await axios.post(
        `${serverUrl}/api/docs/${doc3}/tables/_grist_ACLRules/data/delete`,
        [2],
        chimpy
      );
      assert.equal(resp.status, 200);
  ...
  it("GET /docs/{did}/snapshots retrieves a list of snapshots", async function () {
    const resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.Timesheets}/snapshots`,
      chimpy
    );
    assert.equal(resp.status, 200);
    assert.isAtLeast(resp.data.snapshots.length, 1);
    assert.hasAllKeys(resp.data.snapshots[0], [
      "docId",
      "lastModified",
      "snapshotId",
    ]);
  ...
  it("POST /docs/{did}/states/remove removes old states", async function () {
    ...
    let resp = await axios.get(
      `${serverUrl}/api/docs/${docIds.Timesheets}/states`,
      chimpy
    );
    assert.equal(resp.status, 200);
    ...
    // Remove all but 3.
    resp = await axios.post(
      `${serverUrl}/api/docs/${docIds.Timesheets}/states/remove`,
      { keep: 3 },
      chimpy
    );
    assert.equal(resp.status, 200);
  ...
  it("doc worker endpoints ignore any /dw/.../ prefix", async function () {
    ...
    let resp = await axios.get(
      `${docWorkerUrl}/api/docs/${docIds.Timesheets}/tables/Table1/data`,
      chimpy
    );
    assert.equal(resp.status, 200);
    ...
    resp = await axios.get(
      `${docWorkerUrl}/dw/zing/api/docs/${docIds.Timesheets}/tables/Table1/data`,
      chimpy
    );
    assert.equal(resp.status, 200);
    assert.containsAllKeys(resp.data, ["A", "B", "C"]);
    ...
    if (docWorkerUrl !== homeUrl) {
      resp = await axios.get(
        `${homeUrl}/api/docs/${docIds.Timesheets}/tables/Table1/data`,
        chimpy
      );
      assert.equal(resp.status, 200);
      assert.containsAllKeys(resp.data, ["A", "B", "C"]);
      ...
      resp = await axios.get(
        `${homeUrl}/dw/zing/api/docs/${docIds.Timesheets}/tables/Table1/data`,
        chimpy
      );
      assert.equal(resp.status, 404);
    }
  ...
  describe("webhooks related endpoints", async function () {
    /*
      ...
      */
    async function oldSubscribeCheck(
      requestBody: any,
      status: number,
      ...errors: RegExp[]
    ) {
    ...
    async function postWebhookCheck(
      requestBody: any,
      status: number,
      ...errors: RegExp[]
    ) {
    ...
    it("POST /docs/{did}/tables/{tid}/_subscribe validates inputs", async function () {
      ...
      await oldSubscribeCheck(
        { eventTypes: 0 },
        400,
        /url is missing/,
        /eventTypes is not an array/
      );
      await oldSubscribeCheck({ eventTypes: [] }, 400, /url is missing/);
      await oldSubscribeCheck(
        { eventTypes: [], url: "https://example.com" },
        400,
        /eventTypes must be a non-empty array/
      );
      await oldSubscribeCheck(
        { eventTypes: ["foo"], url: "https://example.com" },
        400,
        /eventTypes\[0] is none of "add", "update"/
      );
      await oldSubscribeCheck({ eventTypes: ["add"] }, 400, /url is missing/);
      await oldSubscribeCheck(
        { eventTypes: ["add"], url: "https://evil.com" },
        403,
        /Provided url is forbidden/
      );
      await oldSubscribeCheck(
        { eventTypes: ["add"], url: "http://example.com" },
        403,
        /Provided url is forbidden/
      ); // not https
      await oldSubscribeCheck(
    ...
    it("POST /docs/{did}/webhooks validates inputs", async function () {
      await postWebhookCheck(
        { webhooks: [{ fields: { tableId: "Table1" } }] },
        400,
        /eventTypes is missing/
      );
      await postWebhookCheck(
        { webhooks: [{ fields: { tableId: "Table1", eventTypes: 0 } }] },
        400,
        /url is missing/,
        /eventTypes is not an array/
      );
      await postWebhookCheck(
        { webhooks: [{ fields: { tableId: "Table1", eventTypes: [] } }] },
        400,
        /url is missing/
      );
      await postWebhookCheck(
      ...
      await postWebhookCheck(
        { webhooks: [{ fields: { tableId: "Table1", eventTypes: ["add"] } }] },
        400,
        /url is missing/
      );
      await postWebhookCheck(
      ...
      await postWebhookCheck(
        {
          webhooks: [
            { fields: { eventTypes: ["add"], url: "https://example.com" } },
          ],
        },
        400,
        /tableId is missing/
    ...
    async function userDeleteCheck(
      user: AxiosRequestConfig,
      webhookId: string,
      status: number,
      ...errors: RegExp[]
    ...
    async function subscribeWebhook(): Promise<SubscriptionInfo> {
      const subscribeResponse = await axios.post(
        `${serverUrl}/api/docs/${docIds.Timesheets}/tables/Table1/_subscribe`,
        { eventTypes: ["add"], url: "https://example.com" },
        chimpy
      );
    ...
    it("DELETE /docs/{did}/tables/webhooks validates inputs for owners", async function () {
      ...
      await check(
        ...
        404,
        /not found/,
        new RegExp(`/api/docs/${docIds.Timesheets}/webhooks/`)
      );
      ...
      await check(
        webhookId,
        404,
        new RegExp(`Webhook not found "${webhookId}"`)
      );
    });
    ...
    it(
      "POST /docs/{did}/webhooks is adding new webhook to table " +
        "and DELETE /docs/{did}/webhooks/{wid} is removing new webhook from table",
      async function () {
    ...
    it("POST /docs/{did}/tables/{tid}/_unsubscribe validates inputs for editors", async function () {
      ...
      let accessResp = await axios.patch(
        `${homeUrl}/api/docs/${docIds.Timesheets}/access`,
        { delta },
        chimpy
      );
      await flushAuth();
      ...
      await check(
        { webhookId: subscribeResponse.webhookId },
        400,
        "Bad request: unsubscribeKey required"
      );
      //wrong unsubscribeKey - it should not be accepted
      //subscribeResponse = await subscribeWebhook();
      await check(
        { webhookId: subscribeResponse.webhookId, unsubscribeKey: "foo" },
        401,
        "Wrong unsubscribeKey"
      );
      //subscribeResponse = await subscribeWebhook();
      ...
      await check(
        {
          webhookId: subscribeResponse.webhookId,
          unsubscribeKey: subscribeResponse.unsubscribeKey,
        },
        200,
        { success: true }
      );
      ...
      await check(
        ...
        404,
        `Webhook not found "${subscribeResponse.webhookId}"`
      );

      // Remove editor access
      delta.users["kiwi@getgrist.com"] = null;
      accessResp = await axios.patch(
        `${homeUrl}/api/docs/${docIds.Timesheets}/access`,
        { delta },
        chimpy
      );
  ...
  describe("Daily API Limit", () => {
    ...
    it("limits daily API usage and sets the correct keys in redis", async function () {
      ...
      const used = 999999;
      let m = moment.utc();
      const currentDay = docPeriodicApiUsageKey(
        docId,
        true,
        docApiUsagePeriods[0],
        m
      );
      const currentHour = docPeriodicApiUsageKey(
        docId,
        true,
        docApiUsagePeriods[1],
        m
      );
      const nextDay = docPeriodicApiUsageKey(
        docId,
        false,
        docApiUsagePeriods[0],
        m
      );
      const nextHour = docPeriodicApiUsageKey(
        docId,
        false,
        docApiUsagePeriods[1],
        m
      ...
      for (let i = 1; i <= 9; i++) {
        ...
        if (i <= 4) {
          ...
          const first = i === 1;
          const day = docPeriodicApiUsageKey(
            docId,
            first,
            docApiUsagePeriods[0],
            m
          );
          const hour = docPeriodicApiUsageKey(
            docId,
            first,
            docApiUsagePeriods[1],
            m
          );
          const minute = docPeriodicApiUsageKey(
            docId,
            true,
            docApiUsagePeriods[2],
            m
          );
        ...
        if (last) {
          ...
          assert.deepEqual(response.data, {
            error: `Exceeded daily limit for document ${docId}`,
          });
        }
      }
    });
    ...
    it("correctly allocates API requests based on the day, hour, and minute", async function () {
      ...
      const currentMinute = docPeriodicApiUsageKey(
        docId,
        true,
        docApiUsagePeriods[2],
        m
      ...
      assert.equal(
        currentMinute,
        `doc-myDocId-periodicApiUsage-1999-12-31T23:59`
      );
      ...
      function check(expected: string[] | undefined) {
        assert.deepEqual(
          getDocApiUsageKeysToIncr(docId, usage, dailyMax, m),
          expected
        );
      }
  ...
  describe("Webhooks", () => {
    ...
    async function subscribe(
      ...
      const { data, status } = await axios.post(
        `${serverUrl}/api/docs/${docId}/tables/${
        ...
        }/_subscribe`,
        {
          ...
          isReadyColumn:
            options?.isReadyColumn === undefined ? "B" : options?.isReadyColumn,
    ...
    async function clearQueue(docId: string) {
      const deleteResult = await axios.delete(
        `${serverUrl}/api/docs/${docId}/webhooks/queue`,
        chimpy
      );
    ...
    before(async function () {
      ...
      serving = await serveSomething((app) => {
        ...
        app.post(
          "/:eventTypes",
          async ({ body, params: { eventTypes } }, res) => {
    ...
    describe("table endpoints", function () {
      ...
      it("delivers expected payloads from combinations of changes, with retrying and batching", async function () {
        ...
        const { subscribeResponses, webhookIds } = await createWebhooks({
          ...
          tableId: "Table1",
          isReadyColumn: "B",
          eventTypesSet: [["add"], ["update"], ["add", "update"]],
        });
        ...
        await axios.post(
          ...
          [
            [
              ...
              "Table1",
              [3, 4, 5, 6],
              { A: [9, 10, 11, 12], B: [true, true, false, false] },
            ],
            [
              "BulkUpdateRecord",
              "Table1",
        ...
        // Unsubscribe
        await Promise.all(
          subscribeResponses.map(async (subscribeResponse) => {
        ...
        const queueRedisCalls = redisCalls.filter(
          (args) => args[1] === "webhook-queue-" + docId
        );
        const redisPushes = _.chain(queueRedisCalls)
          ...
          .value();
        const expectedPushes = _.mapValues(expectedRequests, (value) =>
          _.flatten(value)
        );
        assert.deepEqual(redisPushes, expectedPushes);

        // Check that the events were all removed from the redis queue
        const redisTrims = queueRedisCalls
          .filter((args) => args[0] === "ltrim")
          .map(([, , start, end]) => {
            assert.equal(end, "-1");
            start = Number(start);
        ...
        const expectedTrims = Object.values(redisPushes).map(
          (value) => value.length
        );
        assert.equal(_.sum(redisTrims), _.sum(expectedTrims));
      });
      ...
      [
        ...
        it(ctx.itMsg, async function () {
          ...
          const redisPushIndex = queueRedisCalls.findIndex(
            (args) => args[0] === "rpush"
          );
          ...
          if (ctx.enabled) {
            assert.isAbove(
              redisPushIndex,
              0,
              "Should have pushed events to the redis queue"
            );
          } else {
            assert.equal(
              redisPushIndex,
    ...
    describe("/webhooks endpoint", function () {
      ...
      const waitForQueue = async (length: number) => {
        await waitForIt(
          async () => {
            ...
            assert.equal(
              length,
              _.sum(stats.map((x) => x.usage?.numWaiting ?? 0))
            );
          },
          1000,
          200
      ...
      it("should clear the outgoing queue", async () => {
        ...
        await Promise.all(cleanup.map((fn) => fn())).finally(
          () => (cleanup.length = 0)
        );
        await clearQueue(docId);
      ...
      it("should return statistics", async () => {
        ...
        let unsubscribe1 = await autoSubscribe("200", docId, {
          isReadyColumn: null,
        });
        ...
        await axios.post(
          ...
          [
            [
              ...
              "Table1",
              "C",
              { isFormula: true, formula: formulaEvaluatedAtDocLoad },
            ],
          ],
          chimpy
        );
      ...
      describe("webhook update", function () {
        it("should work correctly", async function () {
          async function check(
            ...
            // update
            const resp = await axios.patch(
              `${serverUrl}/api/docs/${docId}/webhooks/${webhook.webhookId}`,
              fields,
              chimpy
            );
            ...
            assert.equal(
              resp.status,
              status,
              JSON.stringify(pick(resp, ["data", "status"]))
            );
            if (resp.status === 200) {
              stats = await readStats(docId);
              ...
              assert.deepEqual(stats[0].fields, {
                ...expectedFields,
                ...fields,
              });
              if (fields.tableId) {
              ...
              if (error instanceof RegExp) {
                assert.match(
                  resp.data.details?.userError || resp.data.error,
                  error
                );
              } else {
            ...
            // finally  unsubscribe
            const unsubscribeResp = await unsubscribe(
              docId,
              webhook,
              savedTableId
            );
            assert.equal(
              unsubscribeResp.status,
              200,
              JSON.stringify(pick(unsubscribeResp, ["data", "status"]))
            );
            stats = await readStats(docId);
          ...
          await check(
            ...
            200,
            "",
            (expectedFields) => (expectedFields.isReadyColumn = null)
          );
  ...
  describe("Allowed Origin", () => {
    it("should respond with correct CORS headers", async function () {
      ...
      // Target a more realistic Host than "0.0.0.0:port"
      ...
      for (const config of [anonConfig, chimpyConfig]) {
        ...
        for (response of [
          ...
          if (config === anonConfig) {
            ...
            assert.deepEqual(response.data, {
              error: "Credentials not supported for cross-origin requests",
            });
          }
          assert.equal(
            response.headers["access-control-allow-methods"],
      ...
      assert.deepEqual(response.data, {
        error:
          "Unauthenticated requests require one of the headers" +
  ...
  it("GET /docs/{did}/sql is functional", async function () {
    ...
    const resp = await axios.get(
      `${homeUrl}/api/docs/${docIds.Timesheets}/sql?q=${query}`,
      chimpy
    );
    assert.equal(resp.status, 200);
    assert.deepEqual(resp.data, {
  ...
  it("POST /docs/{did}/sql accepts only selects", async function () {
    ...
    await check(
      false,
      "delete from Table1 where id in (select id from Table1) and 'selecty' = 'selecty'"
    );
    await check(
      false,
      "update Table1 set A = ? where 'selecty' = 'selecty'",
      "test"
    );
    ...
    await check(
      true,
      "select * from Table1); delete from Table1 where id in (select id from Table1"
    );
  ...
  it("POST /docs/{did}/sql timeout is effective", async function () {
    const slowQuery =
      "WITH RECURSIVE r(i) AS (VALUES(0) " +
...
async function setupDataDir(dir: string) {
  ...
  await testUtils.copyFixtureDoc(
    "Hello.grist",
    path.resolve(dir, docIds.Timesheets + ".grist")
  );
  await testUtils.copyFixtureDoc(
    "Hello.grist",
    path.resolve(dir, docIds.Bananas + ".grist")
  );
  await testUtils.copyFixtureDoc(
    "Hello.grist",
    path.resolve(dir, docIds.Antartic + ".grist")
  );
