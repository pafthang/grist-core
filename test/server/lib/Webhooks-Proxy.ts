...
const webhooksTestProxyPort = Number(
  process.env.WEBHOOK_TEST_PROXY_PORT || 22335
);
...
describe("Webhooks-Proxy", function () {
  ...
  const tmpDir = path.join(
    tmpdir(),
    `grist_test_${username}_docapi_webhooks_proxy`
  );
  let home: TestServer;
  let docs: TestServer;
  ...
  describe("Proxy is configured", function () {
    runServerConfigurations(
      { GRIST_HTTPS_PROXY: `http://0.0.0.0:${webhooksTestProxyPort}` },
      () => testWebhookProxy(true)
    );
  });

  describe("Proxy not configured", function () {
    runServerConfigurations({ GRIST_HTTPS_PROXY: undefined }, () =>
      testWebhookProxy(false)
    );
  });

  function runServerConfigurations(
    additionaEnvConfiguration: NodeJS.ProcessEnv,
    subTestCall: Function
  ) {
      ...
      async function subscribe(
        ...
        const { data, status } = await axios.post(
          ...
          }/_subscribe`,
          {
            ...
            isReadyColumn:
              options?.isReadyColumn === undefined
                ? "B"
