...
describe("ProxyAgent", function () {
  ...
  it("should be undefined if no proxy is configured", async function () {
    ...
    const httpProxy = proxyAgent(new URL("http://0.0.0.0:3000"));
    const httpsProxy = proxyAgent(new URL("https://0.0.0.0:3000"));
  ...
  it("should be https proxy if proxy is configured and address is https", async function () {
    process.env.GRIST_HTTPS_PROXY = "https://0.0.0.0:9000";
    const httpsProxy = proxyAgent(new URL("https://0.0.0.0:3000"));
  ...
  it("should be https proxy if proxy is configured and address is https", async function () {
    process.env.GRIST_HTTPS_PROXY = "https://0.0.0.0:9000";
    const httpsProxy = proxyAgent(new URL("http://0.0.0.0:3000"));
  ...
  describe("proxy error handling", async function () {
    ...
    beforeEach(async function () {
      ...
      process.env.GRIST_HTTPS_PROXY = `http://0.0.0.0:${port}`;
    });
