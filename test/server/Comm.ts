...
import {
  GristWSConnection,
  GristWSSettings,
} from "app/client/components/GristWSConnection";
import {
  GristClientSocket,
  GristClientSocketOptions,
} from "app/client/components/GristClientSocket";
...
class FakeHosts {
  ...
  public get asHosts() {
    return this as unknown as Hosts;
  }

  public async addOrgInfo<T extends http.IncomingMessage>(
    req: T
  ): Promise<T & RequestOrgInfo> {
...
describe("Comm", function () {
  ...
  before(async function () {
    ...
    const sessionSecret =
      "xkwriagasaqystubgkkbwhqtyyncwqjemyncnmetjpkiwtfzvllejpfneldmoyri";
  ...
  function startComm(methods: { [name: string]: ClientMethod }) {
    ...
    return listenPromise(server.listen(0, "0.0.0.0"));
  }
   ...
   */
  function connect(
    options?: GristClientSocketOptions
  ): Promise<GristClientSocket> {
    const ws = new GristClientSocket(
      "ws://0.0.0.0:" + (server.address() as AddressInfo).port,
      options
    );
  ...
  describe("server methods", function () {
    ...
    it("should return data for valid calls", async function () {
      ...
      assert.equal(
        resp.reqId,
        10,
        `Messages received instead: ${JSON.stringify(messages)}`
      );
    ...
    it("should work for out-of-order calls", async function () {
      ws.send(
        JSON.stringify({ reqId: 30, method: "methodAsync", args: [1, 2] })
      );
      ws.send(
        JSON.stringify({ reqId: 31, method: "methodSync", args: [3, 4] })
      );
    ...
    it("should return error when a call fails", async function () {
      const logMessages = await testUtils.captureLog("warn", async () => {
        ws.send(
          JSON.stringify({ reqId: 40, method: "methodError", args: ["hello"] })
        );
        const messages = await getMessages(ws, 1);
        const resp = messages[0];
    ...
    it("should return error for unknown methods", async function () {
      const logMessages = await testUtils.captureLog("warn", async () => {
        ws.send(
          JSON.stringify({ reqId: 50, method: "someUnknownMethod", args: [] })
        );
        const messages = await getMessages(ws, 1);
        const resp = messages[0];
    ...
    it("should only log warning for malformed JSON data", async function () {
      const logMessages = await testUtils.captureLog(
        "warn",
        async () => {
          ws.send("foobar");
        },
  ...
  describe("reconnects", function () {
    ...
    async function startManagedConnection(methods: {
      ...
      sandbox
        .stub(global as any, "window")
        .value({ gristConfig: { getWorker: "STUB", assignmentId: docId } });

      // We also need to get GristWSConnection to use a custom GristWSSettings object, and to
      // connect to the forwarder's port.
      const docWorkerUrl = `http://0.0.0.0:${forwarderPort}`;
      const settings = getWSSettings(docWorkerUrl);
      const stubGristWsCreate = sandbox
        .stub(GristWSConnection, "create")
        .callsFake(function (this: any, owner) {
          return (stubGristWsCreate as any).wrappedMethod.call(
            this,
            owner,
            settings
          );
    ...
    it("should forward calls on a normal connection", async function () {
      ...
      const resp1 = await cliComm._makeRequest(
        null,
        null,
        "methodSync",
        "foo",
      ...
      const resp2 = await cliComm._makeRequest(
        null,
        null,
        "methodAsync",
        "foo",
    ...
    async function testMissedResponses(sendShouldFail: boolean) {
      ...
      if (sendShouldFail) {
        ...
        sandbox
          .stub(Client.prototype as any, "_sendToWebsocket")
          .onFirstCall()
          .callsFake(() => {
            failedSendCount += 1;
      ...
      const resp2Promise = cliComm._makeRequest(
        null,
        null,
        "testDisconnect",
        "foo",
      ...
      const resp3Promise = cliComm._makeRequest(
        null,
        null,
        "methodAsync",
        "foo",
      ...
      await forwarder.connect();
      assert.deepEqual(await resp2Promise, {
        name: "testDisconnect",
        x: "foo",
        y: 2,
      });
      assert.deepEqual(await resp3Promise, {
        name: "methodAsync",
        x: "foo",
        y: 3,
      });
    ...
    async function testSendOrdering(options: {
      noFailedSend?: boolean;
      closeHappensFirst?: boolean;
      useSmallMsgs?: boolean;
    }) {
      ...
      const stubOnClose = sandbox
        .stub(Client.prototype as any, "_onClose")
        .callsFake(async function (this: Client) {
          if (!options.closeHappensFirst) {
            await delay(10);
          }
      ...
      const stubSendToWebsocket = sandbox
        .stub(Client.prototype as any, "_sendToWebsocket")
        .callsFake(async function (this: Client) {
          try {
            return await (stubSendToWebsocket as any).wrappedMethod.apply(
              this,
              arguments
            );
          } catch (err) {
      ...
      const sendNextMessage = () =>
        sendPromises.push(ssClient.sendMessage(makeMessage(n++) as any));
      ...
      await testUtils.captureLog("warn", async () => {
        ...
        await waitForCondition(
          () =>
            clientConnectSpy.callCount > 0 &&
            clientConnectSpy.lastCall.args[0].needReload === false,
          3000
        );
      });
      ...
      assert.deepEqual(
        await cliComm._makeRequest(null, null, "methodSync", 1, 2),
      ...
      assert.deepEqual(
        eventSpy.getCalls().map((call) => call.args[0].n),
        [n - 1]
      );
    }
  });
  ...
  describe("Allowed Origin", function () {
    ...
    async function checkOrigin(
      ...
      if (allowed) {
        await assert.isFulfilled(
          promise,
          `${headers.host} should allow ${headers.origin}`
        );
      } else {
        await assert.isRejected(
          promise,
          /.*/,
          `${headers.host} should reject ${headers.origin}`
        );
      }
    }
    ...
    it("origin should match base domain of host", async () => {
      await checkOrigin(
        { origin: "https://www.toto.com", host: "worker.example.com" },
        false
      );
      await checkOrigin(
        { origin: "https://badexample.com", host: "worker.example.com" },
        false
      );
      await checkOrigin(
        { origin: "https://bad.com/example.com", host: "worker.example.com" },
        false
      );
      await checkOrigin(
        { origin: "https://front.example.com", host: "worker.example.com" },
        true
      );
      await checkOrigin(
        {
          origin: "https://front.example.com:3000",
    ...
    it("with custom domains, origin should match the full hostname", async () => {
      ...
      await checkOrigin(
        { origin: "https://front.example.com", host: "front.example.com" },
        true
      );
      await checkOrigin(
        { origin: "https://front.example.com:3000", host: "front.example.com" },
        true
      );
    });
  });
...
async function waitForCondition(
  ...
  throw new Error(
    `Condition not met after ${timeoutMs}ms: ${condFunc.toString()}`
  );
}
...
function getWSSettings(docWorkerUrl: string): GristWSSettings {
  ...
  return {
    makeWebSocket(url: string): any {
      return new GristClientSocket(url);
    },
    async getTimezone() {
      return "UTC";
    },
    ...
    updateClientId(did: string, cid: string) {
      clientId = cid;
    },
    ...
    log() {
      (log as any).debug(...arguments);
    },
    warn() {
      (log as any).warn(...arguments);
    },
  };
}
