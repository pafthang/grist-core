...
const debugging =
  isAffirmative(process.env.DEBUG) || isAffirmative(process.env.VERBOSE);
...
import {
  main as mergedServerMain,
  parseServerTypes,
} from "app/server/mergedServerMain";
...
export async function main() {
  ...
  if (!debugging) {
    console.log(`In quiet mode, see http://0.0.0.0:${G.port} to use.`);
