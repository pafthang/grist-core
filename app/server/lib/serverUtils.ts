...
export function getAvailablePort(
  ...
  function checkNext(port: number): Promise<number> {
    if (port > lastPort) {
      throw new Error(
        "No available ports between " + firstPort + " and " + lastPort
      );
    }
    return new bluebird(
      (resolve: (p: number) => void, reject: (e: Error) => void) => {
...
export function getDatabaseUrl(
  ...
  if (options.type === "sqlite") {
    ...
    const creds =
      includeCredentials && options.username
        ? `${options.username}${pass}@`
        : "";
 ...
 */
export async function checkAllegedGristDoc(
  ...
  try {
    ...
    if (
      integrityCheckResults.length !== 1 ||
      integrityCheckResults[0].integrity_check !== "ok"
    ) {
      const uuid = uuidv4();
      ...
      throw new Error(
        `Document failed integrity checks - is it corrupted? Event ID: ${uuid}`
      );
    }
  } finally {
    await db.close();
 ...
 */
export function getTelemetryMetaFromDocSession(
  docSession: OptDocSession
 ...
 */
export async function delayAbort(
  msec: number,
  signal?: AbortSignal
): Promise<void> {
  let cleanup: () => void = () => {};
