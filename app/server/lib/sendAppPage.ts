...
const translate = (req: express.Request, key: string, args?: any) =>
  req.t(`sendAppPage.${key}`, args);
...
export function makeGristConfig(
  options: MakeGristConfigOptions
): GristLoadConfig {
  ...
  const pathOnly =
    ...
    (homeUrl && new URL(homeUrl).hostname === "0.0.0.0") ||
    false;
  ...
  return {
    ...
    enableAnonPlayground: isAffirmative(
      process.env.GRIST_ANON_PLAYGROUND ?? true
    ),
    ...
    maxUploadSizeImport:
      Number(process.env.GRIST_MAX_UPLOAD_IMPORT_MB) * 1024 * 1024 || undefined,
    maxUploadSizeAttachment:
      Number(process.env.GRIST_MAX_UPLOAD_ATTACHMENT_MB) * 1024 * 1024 ||
      undefined,
    timestampMs: Date.now(),
    enableWidgetRepository:
      Boolean(process.env.GRIST_WIDGET_LIST_URL) ||
    ...
    telemetry: server
      ?.getTelemetry()
      .getTelemetryConfig(req as RequestWithLogin | undefined),
...
export function makeMessagePage(staticDir: string) {
  return async (req: express.Request, resp: express.Response, message: any) => {
    const fileContent = await fse.readFile(
      path.join(staticDir, "message.html"),
      "utf8"
    );
    const content = fileContent.replace(
      "<!-- INSERT MESSAGE -->",
      `<script>window.message = ${jsesc(message, {
...
export function makeSendAppPage(opts: {
  ...
  return async (
    ...
    const fileContent =
      options.content ||
      (await fse.readFile(path.join(staticDir, options.path), "utf8"));

    const needTagManager =
      (options.googleTagManager === "anon" && isAnonymousUser(req)) ||
      options.googleTagManager === true;
    const tagManagerSnippet = needTagManager
      ? getTagManagerSnippet(process.env.GOOGLE_TAG_MANAGER_ID)
      : "";
    const staticTag = options.tag || tag;
    // If boot tag is used, serve assets locally, otherwise respect
    // APP_STATIC_URL.
    const staticOrigin =
      staticTag === "boot" ? "" : process.env.APP_STATIC_URL || "";
    const staticBaseUrl = `${staticOrigin}/v/${staticTag}/`;
    const customHeadHtmlSnippet = server.create.getExtraHeadHtml?.() ?? "";
    const warning = testLogin
      ? '<div class="dev_warning">Authentication is not enforced</div>'
      : "";
    ...
    const content = fileContent
      ...
      .replace(
        "<!-- INSERT TITLE SUFFIX -->",
        getPageTitleSuffix(server.getGristConfig())
      )
      .replace(
        "<!-- INSERT BASE -->",
        `<base href="${staticBaseUrl}">` + tagManagerSnippet
      )
      ...
      .replace(
        "<!-- INSERT CONFIG -->",
        `<script>window.gristConfig = ${jsesc(config, {
          isScriptContext: true,
          json: true,
        })};</script>`
      );
...
function logVisitedPageTelemetryEvent(
  req: RequestWithLogin,
  options: LogVisitedPageEventOptions
) {
...
function getFeatures(): IFeature[] {
  ...
  const enabledFeatures =
    process.env.GRIST_UI_FEATURES?.split(",") ?? Features.values;
  return Features.checkAll(difference(enabledFeatures, disabledFeatures));
}

function getPermittedCustomWidgets(
  gristServer?: GristServer | null
): IAttachedCustomWidget[] {
  ...
  const widgetsList =
    process.env.PERMITTED_CUSTOM_WIDGETS?.split(",").map(
      (widgetName) => `custom.${widgetName}`
    ) ?? [];
...
function getDocFromConfig(config: GristLoadConfig): Document | null {
  if (!config.getDoc || !config.assignmentId) {
    return null;
  }
