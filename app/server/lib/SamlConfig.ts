 ...
 * When running on 0.0.0.0 and http, the settings tested were with:
 *   - GRIST_SAML_IDP_SKIP_SLO not set
 *   - GRIST_SAML_SP_HOST=http://0.0.0.0:8080 or 8484
...
export class SamlConfig {
  ...
  public async initSaml(): Promise<void> {
    if (!process.env.GRIST_SAML_SP_HOST) {
      throw new Error("initSaml requires GRIST_SAML_SP_HOST to be set");
    }
    if (!process.env.GRIST_SAML_SP_KEY) {
      throw new Error("initSaml requires GRIST_SAML_SP_KEY to be set");
    }
    if (!process.env.GRIST_SAML_SP_CERT) {
      throw new Error("initSaml requires GRIST_SAML_SP_CERT to be set");
    }
    if (!process.env.GRIST_SAML_IDP_LOGIN) {
      throw new Error("initSaml requires GRIST_SAML_IDP_LOGIN to be set");
    }
    if (!process.env.GRIST_SAML_IDP_LOGOUT) {
      throw new Error("initSaml requires GRIST_SAML_IDP_LOGOUT to be set");
    }
    if (!process.env.GRIST_SAML_IDP_CERTS) {
      throw new Error("initSaml requires GRIST_SAML_IDP_CERTS to be set");
    }
    ...
    const spOptions: saml2.ServiceProviderOptions = {
      ...
      private_key: await fse.readFile(process.env.GRIST_SAML_SP_KEY, {
        encoding: "utf8",
      }),
      certificate: await fse.readFile(process.env.GRIST_SAML_SP_CERT, {
        encoding: "utf8",
      }),
  ...
  public async getLoginRedirectUrl(
    ...
    const { permit: relay_state, samlNameId } = await this._prepareAppState(
      req,
      redirectUrl,
      {
        action: "login",
    ...
    return fromCallback((cb) =>
      sp.create_login_request_url(idp, { relay_state, force_authn }, cb)
    );
  }
  ...
  public async getLogoutRedirectUrl(
    ...
    const {
      ...
      samlNameId,
      samlSessionIndex,
    } = await this._prepareAppState(req, redirectUrl, {
      action: "logout",
      waitMinutes: 1,
    });
    ...
    return fromCallback<string>((cb) =>
      sp.create_logout_request_url(idp, options, cb)
    );
  }
  ...
  public addSamlEndpoints(app: express.Express, sessions: Sessions): void {
    ...
    app.get(
      ...
      expressWrap(async (req, res, next) => {
        res.redirect(
          await this.getLoginRedirectUrl(req, new URL(getOriginUrl(req)))
        );
      })
    );
    ...
    app.post(
      ...
      expressWrap(async (req, res, next) => {
        ...
        const samlResponse: any = await fromCallback((cb) =>
          sp.post_assert(idp, { request_body: req.body }, cb)
        );
        ...
        if (state.action === "login") {
          ...
          if (!samlUser || !samlUser.name_id) {
            log.warn(
              `SamlConfig: bad SAML response: ${JSON.stringify(samlUser)}`
            );
          ...
          log.info(
            `SamlConfig: got SAML response for ${profile.email} (${profile.name}) redirecting to ${redirectUrl}`
          );
   ...
   *
   */
  private async _prepareAppState(
    req: express.Request,
    redirectUrl: URL,
    options: {
    ...
    const permitStore = this._gristServer.getExternalPermitStore();
    const sessionId = this._gristServer
      .getSessions()
    ...
    const scopedSession = this._gristServer
      .getSessions()
    ...
    const permit = await permitStore.setPermit(
      state,
      options.waitMinutes * 60 * 1000
    );
 ...
 */
export async function getSamlLoginSystem(): Promise<
