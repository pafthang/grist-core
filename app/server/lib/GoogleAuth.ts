 ...
 * static URL http://docs.getgrist.com/auth/google (on prod) or http://0.0.0.0:8080/auth/google (dev).
 *
...
const AUTH_SUBDOMAIN = process.env.GRIST_ID_PREFIX
  ? `docs-${process.env.GRIST_ID_PREFIX}`
  : "docs";

/**
 * Return a full url for Google Auth handler. Examples are:
 *
 * http://0.0.0.0:8080/auth/google in dev
...
function getFullAuthEndpointUrl(): string {
  ...
  // if homeUrl is 0.0.0.0 - (in dev environment) - use the development url
  if (homeUrl && new URL(homeUrl).hostname === "0.0.0.0") {
...
export async function googleAuthTokenMiddleware(
  ...
  if (!optStringParam(req.query.code, "code")) {
    throw new ApiError(
      "Google Auth endpoint requires a code parameter in the query string",
      400
    );
  } else {
    try {
...
export function addGoogleAuthEndpoint(
  ...
  if (!process.env.GOOGLE_CLIENT_SECRET) {
    log.warn(
      "Failed to create GoogleAuth endpoint: GOOGLE_CLIENT_SECRET is not defined"
    );
    expressApp.get(
      authHandlerPath,
  ...
  expressApp.get(
    ...
    expressWrap(async (req: express.Request, res: express.Response) => {
      ...
      if (optStringParam(req.query.code, "code")) {
        ...
        messagePage(req, res, {
          code: stringParam(req.query.code, "code"),
          origin: stringParam(req.query.state, "state"),
        });
      } else if (optStringParam(req.query.error, "error")) {
        log.debug(
          "GoogleAuth - response from Google with error code",
          stringParam(req.query.error, "error")
        );
        if (stringParam(req.query.error, "error") === "access_denied") {
          messagePage(req, res, {
            error: stringParam(req.query.error, "error"),
            origin: stringParam(req.query.state, "state"),
          });
        } else {
