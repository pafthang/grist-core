...
if (Piscina.isWorkerThread) {
  throw new Error(
    "ExportXLSX must not be imported from within a worker thread"
  );
}
...
export async function streamXLSX(
  ...
  const testDates = req.hostname === "0.0.0.0";
  ...
  try {
    ...
    rpc.registerImpl<ActiveDocSource>(
      "activeDocSource",
      new ActiveDocSourceDirect(activeDoc, req)
    );
    rpc.on("message", (chunk) => {
