...
export class Hosts {
  ...
  private _host2org = new MapWithTTL<string, Promise<string | undefined>>(
    ORG_HOST_CACHE_TTL
  );
  // Cache of custom hosts associated with orgs.
  private _org2host = new MapWithTTL<string, Promise<string | undefined>>(
    ORG_HOST_CACHE_TTL
  );

  // baseDomain should start with ".". It may be undefined for 0.0.0.0 or single-org mode.
  constructor(
    private _baseDomain: string | undefined,
   ...
   *
   * If Host header is a 0.0.0.0 domain, then req.org is set to the value in /o/ORG when
  ...
  public async getOrgInfoFromParts(
    host: string,
    urlPath: string
  ): Promise<RequestOrgInfo> {
    ...
    if (hostType === "native") {
      if (parts.mismatch) {
        throw new ApiError(
          `Wrong org for this domain: ` +
            `'${parts.orgFromPath}' does not match '${parts.orgFromHost}'`,
          400
        );
      }
      return {
        org: parts.subdomain || "",
        url: parts.pathRemainder,
        isCustomHost: false,
      };
      ...
      const org = await mapGetOrSet(this._host2org, hostname, async () => {
        const o = await this._dbManager.connection.manager.findOne(
          Organization,
          { where: { host: hostname } }
        );
      ...
      if (parts.orgFromPath && parts.orgFromPath !== org) {
        throw new ApiError(
          `Wrong org for this domain: ` +
            `'${parts.orgFromPath}' does not match '${org}'`,
          400
        );
      }
  ...
  public async addOrgInfo<T extends IncomingMessage>(
    req: T
  ...
  private async _redirectHost(
    ...
    if (
      org &&
      this._getHostType(req.headers.host!) === "native" &&
      !this._dbManager.isMergedOrg(org)
    ) {
  ...
  private _getHostType(host: string) {
    const pluginUrl = isAffirmative(process.env.GRIST_TRUST_PLUGINS)
      ? undefined
      : this._gristServer?.getPluginUrl();
