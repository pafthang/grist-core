...
export class FlexServer implements GristServer {
  ...
  constructor(
    ...
    this.host = process.env.GRIST_HOST || "0.0.0.0";
    log.info(
  ...
  public addLogging() {
    ...
    if (process.env.GRIST_LOG_SKIP_HTTP) {
      return;
    }
    ...
    const msg =
      ":logTime :host :method :gristInfo :status :response-time ms - :res[content-length]";
    ...
    this.app.use(
      morganLogger(process.env.GRIST_HOSTED_VERSION ? outputJson : msg, {
  ...
  public get instanceRoot() {
    if (!this._instanceRoot) {
      this._instanceRoot = path.resolve(
        process.env.GRIST_INST_DIR || this.appRoot
      );
  ...
  public addSessions() {
    if (this._check("sessions", "config")) {
      return;
    }
    this.addTagChecker();
    this.addOrg();
  ...
  public async addLandingPages() {
      ...
      : expressWrap(async (req, res, next) => {
          ...
          if (mreq.org && mreq.org.startsWith("o-")) {
            ...
            if (
              ...
              orgInfo.billingAccount.product.features.vanityDomain
            ) {
    ...
    attachAppEndpoint({
      ...
      formMiddleware: [forcedLoginMiddleware],
  ...
  public addComm() {
    if (this._check("comm", "start", "homedb", "config")) {
      return;
    }
  ...
  public async addLoginRoutes() {
    ...
    if (allowTestLogin()) {
      ...
      // For a standard dev environment, it will be accessible at 0.0.0.0:8080/test/login
      // and 0.0.0.0:8080/o/<org>/test/login.  Only available when GRIST_TEST_LOGIN is set.
  ...
  public addEarlyWebhooks() {
    if (this._check("webhooks", "homedb")) {
      return;
    }
  ...
  public addWelcomePaths() {
    ...
    this.app.post(
      ...
      expressWrap(async (req, resp, next) => {
        ...
        this._recordNewUserInfo(row).catch((e) => {
  ...
  public addInstallEndpoints() {
    ...
    this.app.get(
      ...
      ...adminPageMiddleware,
      expressWrap(async (req, resp) => {
        return this.sendAppPage(req, resp, {
          path: "app.html",
          status: 200,
          config: {},
        });
  ...
  public resolveLoginSystem() {
    return process.env.GRIST_TEST_LOGIN
      ? getTestLoginSystem()
  ...
  private async _copyDocToHomeWorkspace(
    ...
    const workspacesQueryResult = await this._dbManager.getOrgWorkspaces(
      getScope(req),
      0
    );
