 ...
 * http://0.0.0.0:8080/testKoForm.html#topTab=4.
...
class Floater extends Disposable implements ContentBox {
  ...
  public create(fillWindow?: boolean) {
    ...
    this.dom = this.autoDispose(
      dom(
        "div.layout_editor_floater",
  ...
  public onInitialMouseMove(mouseEvent: MouseEvent, sourceBox: ContentBox) {
    ...
    sourceBox.leafContent(
      dom(
        "div.layout_editor_empty_space",
...
class DropOverlay extends Disposable {
  ...
  public getAffinity(mouseEvent: MouseEvent) {
    ...
    const top = getFrac(y, this.hBorder!),
      down = getFrac(rect.height - y, this.hBorder!);
    const left = getFrac(x, this.vBorder!),
      right = getFrac(rect.width - x, this.vBorder!);
    const minValue = Math.min(top, down, left, right);

    return minValue === Infinity
      ? -1
      : [top, down, left, right].indexOf(minValue);
  }
}
...
class DropTargeter extends Disposable {
  ...
  public updateTargetHints(
    ...
    if (
      !layoutBox ||
      (layoutBox === this.currentBox && affinity === this.currentAffinity)
    ) {
      return;
    }
    ...
    while (layoutBox) {
      if (upDown === layoutBox.isHBox()) {
        ...
        // is redundant.
          ...
          prevTargetBox !== layoutBox &&
          prevTargetBox !== layoutBox.getSiblingBox(isAfter) &&
          children.length !== 1
        ) {
          targetParts.push({
            box: layoutBox,
    ...
    const self = this;
    this.targetsDom = dom(
      "div.layout_editor_drop_targeter",
      koDom.style(
        pTop,
        innerRect[pTop] -
          outerRect[pTop] +
          (isAfter ? innerRect[pHeight] - totalHeight : 0) +
          "px"
      ),
      targetParts.map((part, index) => {
        const rect = part.box.dom!.getBoundingClientRect();
        return dom(
          "div.layout_editor_drop_target",
          (elem: HTMLDivElement) => {
...
export class LayoutEditor extends Disposable {
  ...
  public create(layout: Layout) {
    ...
    this.measuringBox = this.autoDispose(
      dom("div.layout_editor_measuring_box")
    );
  ...
  public triggerUserEditStart() {
    assert(
      this.lastTriggered === "stop",
      "UserEditStart triggered twice in succession"
    );
  ...
  public triggerUserEditStop() {
    assert(
      this.lastTriggered === "start",
      "UserEditStop triggered twice in succession"
    );
    this.lastTriggered = "stop";
  ...
  public makeResizable(box: LayoutBox) {
    ...
    if (
      G.$(box.dom).resizable("instance") ||
      (box.isHBox() && !this.layout.fillWindow) ||
      box.isLastChild()
    ) {
      return;
    }
  ...
  public onResizeMove(
    ...
    const siblingsFactor =
      (helperObj.sumNext - newSize) / (helperObj.sumNext - helperObj.origSize);
  ...
  public handleMouseDown(event: MouseEvent, elem: HTMLElement) {
    ...
    if (
      event.button !== 0 ||
      target?.classList.contains("ui-resizable-handle")
    ) {
      return;
    }
    if (target?.classList.contains("layout_grabbable")) {
      ...
      assert(
        this.originalBox,
        "MouseDown on element without an associated layoutBox"
      );
  ...
  public updateTargets(event: MouseEvent) {
    ...
    if (elem) {
      ...
      this.dropTargeter.updateTargetHints(
        hoverBox,
        affinity,
        this.dropOverlay,
        this.targetBox!
...
function resizeLayoutBox(layoutBox: LayoutBox, sizeRect: string | DOMRect) {
  ...
  if (layoutBox.isHBox()) {
    layoutBox.dom!.style.height = reset
      ? ""
      : collapse
      ? "0px"
      : (sizeRect as DOMRect).height + "px";
  } else {
    layoutBox.dom!.style.width = reset
      ? ""
      : collapse
      ? "0px"
      : (sizeRect as DOMRect).width + "px";
  }
  layoutBox.dom!.style.opacity = collapse ? "0.0" : "1.0";
}

function rectDesc(rect: string | DOMRect) {
  return typeof rect === "string"
    ? rect
 ...
 */
function resizeLayoutBoxSmoothly(
  layoutBox: LayoutBox,
  startRect: string | DOMRect,
  endRect: string | DOMRect
) {
  ...
  return new Promise(function (resolve, reject) {
    dom.once(layoutBox.dom, "transitionend", function () {
      resolve();
    });
    ...
    .catch(Promise.TimeoutError, function () {
      console.error(
        "LayoutEditor.resizeLayoutBoxSmoothly %s %s->%s: transition didn't run",
        layoutBox,
        rectDesc(startRect),
        rectDesc(endRect)
      );
