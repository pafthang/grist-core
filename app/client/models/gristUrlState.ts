...
export function urlState(): UrlState<IGristUrlState> {
  return (
    _urlState ||
    (_urlState = new UrlState(window, new UrlStateImpl(window as any)))
  );
}
...
export function getMainOrgUrl(): string {
  return urlState().makeUrl({});
}

// When on a document URL, returns the URL with just the doc ID, omitting other bits (like page).
export function getCurrentDocUrl(): string {
  return urlState().makeUrl({ docPage: undefined });
}
...
export function getLoginOrSignupUrl(
...
function _getCurrentUrl(): string {
  ...
  if (FINAL_PATHS.some((final) => pathname.endsWith(final))) {
    return "/";
  }
...
function _getLoginLogoutUrl(
  ...
  if (srcDocId) {
    startUrl.searchParams.set("srcDocId", srcDocId);
  }
  if (nextUrl) {
...
function _buildUrl(page?: string): URL {
  ...
  startUrl.pathname =
    addOrgToPath("", window.location.href, true) + "/" + (page ?? "");
  startUrl.search = "";
  startUrl.hash = "";
  return startUrl;
}
...
export class UrlStateImpl {
   ...
   */
  public updateState(
    prevState: IGristUrlState,
    newState: IGristUrlState
  ): IGristUrlState {
    const keepState =
      newState.org ||
      newState.ws ||
  ...
  public needPageLoad(
    ...
    const accountReload =
      Boolean(prevState.account) !== Boolean(newState.account);
    // Reload when moving to/from a billing page.
    const billingReload =
      Boolean(prevState.billing) !== Boolean(newState.billing);
    // Reload when moving to/from an activation page.
    const activationReload =
      Boolean(prevState.activation) !== Boolean(newState.activation);
    // Reload when moving to/from a welcome page.
    const welcomeReload =
      Boolean(prevState.welcome) !== Boolean(newState.welcome);
    // Reload when link keys change, which changes what the user can access
    const linkKeysReload = !isEqual(
      prevState.params?.linkParameters,
      newState.params?.linkParameters
    );
    // Reload when moving to/from the Grist sign-up page.
    const signupReload =
      [prevState.login, newState.login].includes("signup") &&
      prevState.login !== newState.login;
    // Reload when moving to/from the support Grist page.
    const adminPanelReload =
      Boolean(prevState.adminPanel) !== Boolean(newState.adminPanel);
    return Boolean(
      orgReload ||
        accountReload ||
        billingReload ||
 ...
 */
export function sameDocumentUrlState(
  urlValue: CellValue
): IGristUrlState | null {
