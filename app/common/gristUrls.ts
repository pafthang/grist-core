...
import {
  ...
  isAffirmative,
  removePrefix,
} from "app/common/gutil";
import { LocalPlugin } from "app/common/plugin";
import { StringUnion } from "app/common/StringUnion";
import { TelemetryLevel } from "app/common/Telemetry";
import {
  ThemeAppearance,
  ThemeAppearanceChecker,
  ThemeName,
  ThemeNameChecker,
...
export const WelcomePage = StringUnion(
  "teams",
  "signup",
  "verify",
  "select-account"
...
export const LoginPage = StringUnion(
  "signup",
  "login",
  "verified",
  "forgot-password"
...
export const commonUrls = {
  ...
  helpAssistantDataUse:
    "https://support.getgrist.com/ai-assistant/#data-use-policy",
  ...
  helpFilterButtons:
    "https://support.getgrist.com/search-sort-filter/#filter-buttons",
  ...
  helpUnderstandingReferenceColumns:
    "https://support.getgrist.com/col-refs/#understanding-reference-columns",
  helpTriggerFormulas:
    "https://support.getgrist.com/formulas/#trigger-formulas",
  helpTryingOutChanges:
    "https://support.getgrist.com/copying-docs/#trying-out-changes",
  ...
  forms:
    "https://www.getgrist.com/forms/?utm_source=grist-forms&utm_medium=grist-forms&utm_campaign=forms-footer",

  basicTutorial: "https://templates.getgrist.com/woXtXUBmiN5T/Grist-Basics",
  basicTutorialImage:
    "https://www.getgrist.com/wp-content/uploads/2021/08/lightweight-crm.png",
...
function isDocInternalUrl(host: string) {
  if (!process.env.APP_DOC_INTERNAL_URL) {
    return false;
  }
...
export function getOrgUrlInfo(
  newOrg: string,
  currentHost: string,
  options: OrgUrlOptions
): OrgUrlInfo {
...
export function encodeUrl(
  gristConfig: Partial<GristLoadConfig>,
  ...
  if (state.doc) {
    if (state.api) {
      ...
      parts.push(
        `${encodeURIComponent(state.doc)}/${encodeURIComponent(state.slug)}`
      );
    } else {
    ...
    parts.push(
      `forms/${encodeURIComponent(state.form.shareKey)}/${encodeURIComponent(
        state.form.vsId
      )}`
    );
  ...
  if (state.account) {
    parts.push(
      state.account === "account" ? "account" : `account/${state.account}`
    );
  }

  if (state.billing) {
    parts.push(
      state.billing === "billing" ? "billing" : `billing/${state.billing}`
    );
  }

  if (state.activation) {
    parts.push(state.activation);
  }

  if (state.login) {
    parts.push(state.login);
  }
  ...
  if (state.adminPanel) {
    parts.push(state.adminPanel);
  }

  const queryParams = pickBy(
    state.params,
    (v, k) => k !== "linkParameters"
  ...
  if (
    state.hash &&
    (state.hash.rowId || state.hash.popup || state.hash.recordCard)
  ) {
    ...
    for (const key of ["sectionId", "rowId", "colRef"] as Array<
      keyof HashLink
    >) {
...
export function decodeUrl(
  ...
  if (map.has("ws")) {
    state.ws = parseInt(map.get("ws")!, 10);
  }
  if (map.has("doc")) {
    ...
    if (map.has("slug")) {
      state.slug = map.get("slug");
    }
    if (map.has("p")) {
      state.docPage = parseDocPage(map.get("p")!);
    }
    if (map.has("f")) {
      state.form = { vsId: parseInt(map.get("f")!, 10) };
    }
  } else {
    if (map.has("p")) {
      const p = map.get("p")!;
  ...
  if (map.has("m")) {
    state.mode = OpenDocMode.parse(map.get("m"));
  }
  if (map.has("account")) {
    state.account = AccountPage.parse(map.get("account")) || "account";
  }
  if (map.has("billing")) {
    state.billing = BillingSubPage.parse(map.get("billing")) || "billing";
  }
  if (map.has("activation")) {
    state.activation =
      ActivationPage.parse(map.get("activation")) || "activation";
  }
  if (map.has("welcome")) {
    state.welcome = WelcomePage.parse(map.get("welcome"));
  }
  if (map.has("admin")) {
    state.adminPanel = AdminPanelPage.parse(map.get("admin")) || "admin";
  }
  if (sp.has("planType")) {
    state.params!.planType = sp.get("planType")!;
  }
  if (sp.has("billingPlan")) {
    state.params!.billingPlan = sp.get("billingPlan")!;
  }
  ...
  if (sp.has("embed")) {
    ...
    if (embed && !state.mode && !state.fork) {
      state.mode = "view";
    }
    // Turn on single page style if no style has been specified.
    if (embed && !state.params!.style) {
      state.params!.style = "singlePage";
    }
  }
  ...
  for (const [k, v] of sp.entries()) {
    if (k.endsWith("_")) {
      if (!state.params!.linkParameters) {
        state.params!.linkParameters = {};
      }
  ...
  if (location.hash) {
    ...
    if (
      hashMap.has("#") &&
      ["a1", "a2", "a3"].includes(hashMap.get("#") || "")
    ) {
      const link: HashLink = {};
      const keys = ["sectionId", "rowId", "colRef"] as Array<
        "sectionId" | "rowId" | "colRef"
      >;
      for (const key of keys) {
        ...
        if (key === "rowId" && value === "new") {
          ...
          const rowIdParts = value
            .split("-")
            .map((p) => (p === "new" ? p : parseInt(p, 10)));
...
export function userOverrideParams(
  email: string | null,
  extraState?: IGristUrlState
) {
...
const localhostRegex = /^0.0.0.0(?::(\d+))?$/i;
 ...
 */
export function parseSubdomainStrictly(host: string | undefined): {
  org?: string;
  base?: string;
} {
  if (!host) {
    throw new Error("host not known");
  }
...
export const GristDeploymentTypes = StringUnion(
  "saas",
  "core",
  "enterprise",
  "electron",
...
export interface ActivationState {
  trial?: {
    // Present when installation has not yet been activated.
...
export function isOrgInPathOnly(host?: string): boolean {
  if (isClient()) {
    ...
    if (host && host.match(localhostRegex)) {
      return true;
    }
 ...
 */
export function extractOrgParts(
  ...
  if (part.value) {
    ...
    const mismatch = Boolean(
      orgFromHost && orgFromPath && orgFromHost !== orgFromPath
    );
    const subdomain = mismatch ? null : orgFromPath;
    return {
      orgFromHost,
      orgFromPath,
      pathRemainder: part.path,
      mismatch,
      subdomain,
    };
  }
  return {
    orgFromHost,
    orgFromPath: null,
    pathRemainder: reqPath,
    mismatch: false,
    subdomain: orgFromHost,
  };
}
 ...
 */
export function parseFirstUrlPart(
  tag: string,
  path: string
...
export function parseUrlId(urlId: string): UrlIdParts {
  ...
  return {
    ...
    forkId: bareParts[1],
    forkUserId:
      bareParts[2] !== undefined ? parseInt(bareParts[2], 10) : undefined,
    snapshotId,
    shareKey,
  };
}
...
export function buildUrlId(parts: UrlIdParts): string {
  let token = [parts.trunkId, parts.forkId, parts.forkUserId]
  ...
  if (parts.snapshotId) {
    ...
    const codedSnapshotId = encodeURIComponent(parts.snapshotId)
      .replace(
        /[_.!~*'()-]/g,
        (ch) => `_${ch.charCodeAt(0).toString(16).toUpperCase()}`
      )
      .replace(/%/g, "_");
...
function shouldIncludeSlug(doc: { id: string; urlId: string | null }): boolean {
  if (!doc.urlId || doc.urlId.length < MIN_URLID_PREFIX_LENGTH) {
    return false;
  }
...
// not be used.
export function getSlugIfNeeded(doc: {
  id: string;
  urlId: string | null;
  name: string;
}): string | undefined {
  if (!shouldIncludeSlug(doc)) {
    return;
  }
...
export interface UrlTweaks {
   ...
   */
  preDecode?(options: { url: URL }): void;
}
